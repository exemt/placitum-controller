...
const LINE = 20;
const TAB = "    ";
/** Рамка сверху и снизу плюс поля 10px -- всё, что окно занимает мимо строк. */
const CHROME = 22;
/** Хвост под окном: те же 24px, что страница держит полем снизу. */
const FILL_TAIL = 24;
...
export function NginxEditor({
  ...
  readOnly?: boolean;
  wide?: boolean;
  /** Тянуть окно до низа страницы вместо роста по числу строк. */
  fill?: boolean;
}) {
  ...
  /*
    Высота при `fill` -- та, при которой страница перестаёт прокручиваться: от
    верха окна до низа экрана, минус поле страницы. Верх берём в координатах
    документа, а не экрана: иначе прокрученная страница мерила бы саму себя и
    окно росло бы с каждым замером.
  */
  ...
  /*
    Меряем на каждый проход, а не однажды: над окном стоят алерты разбора, они
    приходят и уходят вместе с текстом, и высота, снятая один раз, оставила бы
    под окном дыру ровно в их рост. Повтор бесплатен -- ту же высоту гасит
    сравнение в setFilled.
  */
  useLayoutEffect(() => {
    if (fill !== true) {

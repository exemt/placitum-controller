...
export type PaginatorProps = {
  /**
   * Состояние из `usePager`: страница, размер, итог и оба обработчика.
   * Отдельные пропсы ниже остаются -- ими правят то, что считает вызывающий
   * (серверные списки держат страницу у себя).
   */
  pager?: PagerState;
  /** Только размер (`usePageSize`) -- когда страницу режет сервер. */
  size?: PageSizeState;
  page?: number;
  pageSize?: number;
  total?: number;
  ...
  /**
   * Блок, по высоте которого считают размер страницы: id, узел или ref.
   * Внутри `DataTable` не нужен -- скроллер приходит контекстом.
   */
  container?: AutoTarget;
  /** Считать размер по высоте. По умолчанию -- как сказало состояние. */
  auto?: boolean;
  autoMin?: number;
  autoMax?: number;
  /** Что вычесть сверх шапки таблицы. */
  autoReserve?: number;
  /** Своя высота строки, если строки одинаковы, а таблица пока пуста. */
  rowHeight?: number;
...
export function Paginator({
  ...
  /*
   * Расчёт живёт в пагинаторе, а не в странице: только он стоит под таблицей
   * и знает её скроллер -- страница про разметку не спрашивает.
   */
    ...
    >
      ...
      {sizeVisible && changeSize !== undefined && (
        <Stack direction="row" spacing={0.75} sx={{ alignItems: "center" }}>
          ...
          >
            {/*
              «Авто» -- первый пункт и состояние по умолчанию: размер страницы
              считает высота блока. Ручное число его отключает до перезагрузки
              страницы, обратно -- этим же пунктом.
            */}
            {state !== undefined && (

/*
 * Темп маршрута в ячейке таблицы.
 *
 * Тот же язык, что у страницы состояния: число сверху, полоса под ним, цвета
 * классов ответа -- CODE_PARTS, один список на весь UX. Полоса составная, а не
 * заполненная: у темпа нет потолка, относительно которого «сколько занято»
 * что-то значило бы, зато есть состав -- и именно он отвечает на вопрос, ради
 * которого на эту колонку смотрят: сколько из этого ошибки.
 *
 * Прочерк вместо нуля, пока снимка нет. «Тихо» и «неизвестно» -- разные
 * состояния, и ноль на месте второго врёт ровно в тот момент, когда оператор
 * смотрит сюда, потому что что-то сломалось.
 */

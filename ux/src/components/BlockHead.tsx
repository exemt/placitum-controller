...
/**
 * Плашка состояния — первая колонка шапки. Двухбуквенный код одной ширины у
 * всех карточек: ok — всё сведено, er — отказ или деградация, dr — конфиг
 * разошёлся. Расшифровка — в title.
 */
...
/** Красный счётчик в шапке: сколько записей в кольце ошибок участника. */
...
/** Порог, с которого перекос часов производителя показывается в шапке. */
const SKEW_BADGE_MS = 3000;

/**
 * Часы производителя разошлись с контроллером. Кривое время ломает TTL и
 * дедлайны, а ищут его всегда последним — поэтому бейдж, а не строка в теле.
 */
...
/**
 * Метрики свёрнутой шапки в фиксированных колонках: один и тот же слот во
 * всех карточках занят одним и тем же смыслом (работа, работа, CPU, память,
 * канал). Пустой слот остаётся пустым — колонки совпадают по вертикали, и
 * страница сравнивается взглядом сверху вниз, а не карточка за карточкой.
 */
  ...
  return (
    <Box
      sx={{
        ...
        // Колонки равные, но эластичные: на узком экране все карточки
        // ужимаются одинаково, и вертикальное выравнивание сохраняется.
    ...
    >
      {shown.map((slot, index) => (
        // eslint-disable-next-line react/no-array-index-key -- слоты позиционные
...
/**
 * Рамка карточки. `tone` красит левую кромку в состояние участника — это
 * первый ответ страницы на «где болит», видный до чтения шапки.
 */
...
/**
 * Шапка карточки флота: первым идёт состояние — плашка одной ширины у всех
 * карточек, колонкой по левому краю страницы; за ней название, серый label и
 * счётчик ошибок; справа метрики в фиксированных слотах.
 */

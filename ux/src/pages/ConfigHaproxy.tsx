/*
  «Конфигурация → haproxy»: балансировщик перед краями.

  Это отдельный документ, а не часть конфигурации nginx, и отдельная страница
  именно поэтому: у него своя кнопка «сохранить» и своя «отправить».
  Контроллер собирает из документа haproxy.cfg целиком; агент на узле
  проверяет его `haproxy -c` и перечитывает мастер без разрыва соединений.

  На странице только то, что печатается в конфиг. Бутстрап агента (адрес шины,
  пути, pidfile) остаётся окружением контейнера: им агент дотягивается до
  самой шины, и доставить его шиной нельзя по кругу зависимостей.
*/
...
/**
 * Разделы страницы -- вкладки, как у агента: на экране одна таблица, а что в
 * ней -- видно по ряду сверху. Серверы -- своя вкладка: таблица данных не
 * вкладывается в таблицу настроек.
 */
...
/** Пустые ветки убираются: `{process:{}}` и «ничего не задано» -- одно и то же. */
...
/**
 * Строки server backend'а: таблица данных с черновой строкой внизу, как у
 * форматов логов (`config/HttpCatalogs.tsx`). Правка в ячейке меняет черновик
 * страницы, сохраняет его общая строка состояния.
 */
    ...
    <SectionBleed>
      <Table size="small" sx={serversTableSx}>
        ...
        <TableBody>
          ...
          {/* Черновая строка: новый сервер набирается здесь, а не пустой записью. */}
          <TableRow>
            <TableCell sx={cellSx}>
              <InputBase
                inputRef={nameRef}
...
export default function ConfigHaproxy() {
  ...
  // Канал в общем механизме сходимости: состояние, причина и право нажать
  ...
  return (
    <Stack spacing={2}>
      ...
      <LayerCard flush title={item?.label ?? ""} hint={item?.hint}>
        {tab === "nodes" ? (
          /* Узлы -- таблица данных: сюда смотрят, а не правят. */
          <SectionBleed scroll>
          <Table size="small">
            <TableHead>
              <TableRow>

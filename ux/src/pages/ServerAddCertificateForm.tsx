...
/**
 * Какие типы сертификата годятся для каждого назначения привязки.
 * Зеркало KIND_ACCEPTS контроллера (src/model/listen.ts): ssl_certificate --
 * только серверная пара, ssl_client_certificate -- только корень mTLS,
 * ssl_trusted_certificate принимает и то и другое.
 */
...
export function ServerAddCertificateForm({
  ...
  // Список сужается под выбранное назначение: серверную пару нельзя повесить
  // в ssl_client_certificate, а корень mTLS -- в ssl_certificate. Контроллер
  // это отклонит (kind_mismatch), но выбирать заведомо неверное оператору
  // предлагать незачем.
  ...
  // Смена назначения может выбросить выбранную строку из списка -- тогда
  // берём первую подходящую, а не оставляем невалидный id в форме.
  useEffect(() => {
    if (!open) {
      return;
    }
  ...
  function submit() {
    ...
    // Как и у портов: диалог пополняет черновик, пишет карточка.

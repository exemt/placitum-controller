/*
  «Общая»: то, что настраивает процесс, а не блок `http {}`.

  Скелет файла (`load_module`, `worker_processes`, `events {}`, `include`)
  стоит выше http {} буквально -- агент пишет вывод компилятора целиком в
  nginx.conf и проверяет `nginx -t -c`. Шина, shm-зона и сокет вердиктов
  печатаются внутри http {}, но действуют на весь процесс: на серверы и пути
  они не наследуются и переопределить их маршрут не может. На странице http
  они выглядели настройкой маршрута, которой не являются.

  Документ тот же, что у страницы http, и сохраняется целиком -- см.
  `config/http-draft.tsx`.

  Каждая секция -- одна `SettingsTable`; группы -- подзаголовки строк.
  Подстановки и единицы -- справа, в строке редактора (`settings-table.tsx`).
*/
...
/**
 * Разделы страницы. Скелет файла -- это nginx, шина и зона -- это WAF: два
 * разных набора, которые правят разные руки, и лентой секций они стояли одной
 * стопкой. Вкладки те же, что на странице http.
 */
type GeneralView = "waf" | "nginx" | "preview";

/** Второй слой: группы директив выбранного раздела. */
...
/**
 * `error_log <path> [level]` главного контекста хранится строкой. Разбираем на
 * путь и уровень, чтобы уровень выбирался селектором, а не вспоминался.
 */
...
export default function ConfigGeneral() {
  ...
  /*
    Строки выбранной группы. Заголовка группы внутри нет: её имя уже стоит на
    вкладке, а вторая подпись тем же начертанием читалась повтором.
  */
  const fields = () => {
    switch (tab) {
      case "mainEvents":
        return (
          <>
            ...
            />
            {/* Умолчания у `use` нет: пусто -- nginx выбирает метод сам. */}
            <Choice
              t={t}
              optional
      ...
      case "shm":
        return (
          <>
            ...
            />
            {/*
              Умолчания у сокета нет: при включённом waf компилятор без него
              откажет (agent_socket_required), поэтому поле стоит рядом с
              зоной, а не в «ещё».
            */}
            <Text
      ...
      case "bus":
        return (
          <>
            {/*
              Адрес шины -- развёртывание, а не документ пространства: тот же
              NATS слушают агент (agent.conf) и инспекторы, и правка одного из
              трёх развела бы их молча. Контроллер печатает свой
              CONTROLLER_NATS_URL; здесь он виден, но не правится. По той же
              границе, что реквизиты S3 у агента, отсюда убраны user/pass/token:
              контроллер называет шину, но открывает её развёртывание.
            */}
            ...
            {/*
              Оба Redis -- тоже развёртывание, на чтение: обменник тел печатается
              в waf_store, внутренний (блобы поколений, корзины, роастер) модулю
              не нужен вовсе. Пароль из адреса контроллер не отдаёт.
            */}
      ...
      default:
        return (
          <>
            ...
            />
            {/*
              `auto` или число: одно поле, подстановки справа. Число уходит
              числом, чтобы компилятор не цитировал строку.
            */}
            <Text
              optional
              fallback="auto"
            ...
            />
            {/* Путь -- текстом, уровень -- селектором у правого края. */}
            <Text
              optional
            ...
            />
            {/*
              Копия главного журнала в сокет агента -- оттуда в «Журналы».
              Ключа нет -- копия есть; «выкл» оставляет только файл.
            */}
            <Tri
              t={t}
              fallback="on"
  ...
  return (
    <Stack spacing={2}>
      ...
      {/*
        Навигация в два слоя, как на странице http: раздел, под ним -- его
        группа. Скелет файла и шина стояли двумя аккордеонами, и до пределов
        сообщений приходилось раскрывать оба.
      */}
      <LayerBar>
      ...
      {view === "preview" ? (
        /* Скелет и http {}; серверы с апстримами -- заглушками по uuid. */
        <PreviewDock
          scope={scope}
        ...
        /*
          Скелет файла -- не блок http {}: агент пишет вывод компилятора целиком
          в nginx.conf и проверяет `nginx -t -c`. Шина и зона печатаются внутри
          http {}, но действуют на весь процесс и на серверы не наследуются.
        */
        <LayerCard flush title={item?.label ?? ""} hint={item?.hint}>
          {/*
            Вкладка переменных -- один список, и он стоит таблицей во всю
            секцию, а не строкой настройки с парами внутри ячейки.
          */}
          {tab === "vars" ? (
            <VarsCatalog
              rows={vars}

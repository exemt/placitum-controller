...
/*
 * Предпросмотр страницы отказа: черновик из формы прогоняется через мини-SSI
 * и показывается так, как его увидит клиент. Оператор правит шаблон вслепую
 * иначе: ветки по $waf_deny_scope в текстовом поле не видно, а проверять на
 * стенде каждую правку -- значит не проверять ни одной.
 *
 * Семантика повторяет nginx (проверено на 1.28.0, см. docs/deny-pages.md):
 *
 * - голый `$var` в expr -- «есть ли значение»; `$x != ''` здесь не работает
 *   и в шаблонах не используется;
 * - правая часть сравнения берётся буквально, без снятия кавычек;
 * - `default=` у echo подставляется, когда переменной нет;
 * - подстановка экранируется (encoding="entity" -- умолчание nginx).
 *
 * Это предпросмотр, а не валидатор: include, config и прочие директивы SSI
 * молча пропускаются, как незнакомые страницам отказа.
 */
...
export function renderSsi(src: string, vars: Record<string, string>): string {
  const out: string[] = [];
  // Стек условных веток: taken -- ветка уже была взята, on -- текущая активна.
...
/*
 * Сценарии -- типовые причины отказа. Каждый отвечает на вопрос «что скажет
 * страница, когда...», а не перечисляет переменные: переменные -- деталь
 * доставки, оператор думает случаями.
 *
 * silent -- запись каталога заперла все params= (или запрос шёл мимо модуля):
 * страница обязана оставаться осмысленной и с пустыми руками.
 */
const BASE = {
...
export function PagePreviewDialog({
  ...
  name: string;
  /** Имя типа содержимого: html, json, xml, text. */
  typeName: string;
  body: string;
  onClose: () => void;
}) {

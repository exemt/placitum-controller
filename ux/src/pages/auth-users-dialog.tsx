/**
 * Пользователи списка провайдера local -- отдельным окном.
 *
 * В форме профиля остался один селектор списка: записей в нём бывают сотни,
 * а вываленные под поле строки отодвигали за край панели всё остальное --
 * сессию, ворота, приложение. Окно открывают шестерёнкой рядом с селектором,
 * и здесь у списка есть то, чего в столбце полей быть не могло: поиск по
 * логину и страницы.
 *
 * Список читается и правится тем же API, что любой другой набор строк: своей
 * ручки у калитки ровно одна -- посчитать bcrypt (см. `addAuthUser`).
 */
...
export type AuthUsersPayload = {
  scope: string;
  datasetId: string;
  /** Имя списка: приписка в шапке окна, чтобы не правили вслепую. */
  name: string;
};

/**
 * Высота тела окна. Задана числом, потому что по высоте скроллера пагинатор
 * считает размер страницы: тело, растущее по содержимому, считало бы его по
 * самому себе.
 */
const BODY_H = 380;
...
/*
 * Запись списка: логин и группы читаются из строки, хеш наружу не показывается
 * -- по нему подбирают офлайн, а глазами он всё равно ничего не говорит.
 * Строку, которая не разобралась, показываем как есть: список правят и мимо
 * этого окна, и молчать про мусор нельзя.
 */
...
export function AuthUsersDialog() {
  ...
  /*
   * Записи тянутся на открытие окна, а не вместе с профилем: панели они
   * больше не нужны, а список бывает длинным.
   */
  useEffect(() => {
  ...
  /* В сторе лежат записи последнего запрошенного списка -- чужие не показываем. */
...
/**
 * Новый пользователь: логин, пароль и группы.
 *
 * Пароль уходит на ручку контроллера и возвращается строкой `login:bcrypt`:
 * в панели его не хешируют и нигде не держат, поле пустеет вместе с окном.
 */
function AuthUserAddDialog() {
  ...
  /* Пароль короче восьми знаков контроллер не примет: кнопка серая до него. */
  const ready =

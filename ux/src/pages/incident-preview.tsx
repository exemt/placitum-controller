...
/**
 * Клик по паре заголовка или параметра в срезе — просится в фильтр списка. У
 * урезанной пары значения нет: точное сравнение с префиксом не нашло бы даже
 * эту запись, поэтому в фильтр уходит одно имя.
 */
...
/**
 * Срез запроса справа от инспекторов. Сверху — вид, источник и скачать.
 * Срок архива — в подсказке у archive. Причина итога живёт слева.
 * Нет записи в архиве — archive и скачать выключены. Archive —
 * простой текст, без кликабельных строк; догружается окнами по scroll.
 */
...
export function PreviewPanel({
  ...
  /*
   * Фаза берётся у самой строки, а не подставляется «request». У запроса и
   * ответа общий ray и две записи: срез ответа лежит в своей, и без фазы
   * бэкенд отдал бы не ту -- открыв строку фазы ответа, оператор смотрел бы на
   * заголовки запроса и не понимал, почему в теле не та утечка.
   */
  const phase = row.phase || "request";
  // Кадр внутри соединения: под ray лежат все кадры, свой -- по адресу.
  ...
  /*
   * У кадра один срез -- полезная нагрузка, у сессии среза нет вовсе: она
   * итог соединения, снимать там нечего. Вкладки заголовков и параметров у
   * обеих фаз показывали бы пустоту с намёком, что что-то не снялось.
   */
  ...
  /*
   * У ответа нет строки запроса: она объект фазы запроса (capture.md), и
   * вкладка «параметры» у записи ответа показывала бы пустоту с намёком, что
   * что-то не снялось.
   */
  useEffect(() => {
        ...
        >
          {source === "archive" ? (
            ...
            <BodyPreview
              ...
              error={event.error}
              /*
               * У кадра два среза: body_preview по бюджету waf_preview frame
               * (та же колонка, что у тела, по ней же ищут) и короткий
               * payload_preview, который пишется всегда. Показывается
               * настроенный, короткий -- когда превью на маршруте нет.
               */
              text={
                phase === "frame"
...
/*
 * Клик по паре просится в фильтр списка: то же имя (и то же значение, если
 * оно совпало) должно находиться и в остальных записях, а не только читаться
 * тут. Заголовки сравниваются без учёта регистра — так их привёл логгер;
 * имя параметра — с учётом, регистр в query string значим.
 */
...
async function downloadArchive(
  ...
  try {
    ...
    // Карточка не блокируется: скачивание — побочный жест, а не разбор.
  }
}

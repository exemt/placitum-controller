/**
 * Загрузка выгрузки MaxMind со страниц «Гео» и «ASN».
 *
 * Кнопка в полосе открывает окно выбора файла; файл уходит контроллеру одним
 * телом, а дальше страница следит за задачей и за кодером: сверка каталога
 * идёт минутами, а кодер переключается на копию сам, когда контроллер назовёт
 * ему файл. Полоса показывает то, что нужно видеть без окна: какая выгрузка
 * стоит, идёт ли загрузка и сколько копий кодера уже на новом файле.
 */
...
/* Пока задача идёт -- раз в секунду: ход виден числом сетей. */
const POLL_RUNNING_MS = 1000;
/* Кодер качает копию и собирает таблицы -- секунды; чаще спрашивать незачем. */
const POLL_CODER_MS = 2000;
/* Кодер не догнал за столько -- перестать спрашивать: полоса и так скажет «не догнал». */
...
export function useGeoImport(
  kind: GeoKind,
  scope: string | null,
  /** Загрузка кончилась успешно: странице перечитать список. */
  onLoaded: () => void,
): {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Отказы страница показывает своим списком; полоса остаётся прежней.
    }
  }, [scope]);
  ...
  useEffect(() => {
    /*
     * Кодера ждут не только после своей загрузки: страницу открыли сразу после
     * чужой (или своей в другой вкладке) -- счёт от времени загрузки файла.
     */
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- итог считается по смене состояния задачи
  }, [state]);
...
/** Код отказа контроллера -- во фразу; незнакомый показывается как есть. */
function errorText(
  t: Translate,
  kind: GeoKind,

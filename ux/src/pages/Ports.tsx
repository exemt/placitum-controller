...
/** Подстановки адреса: все интерфейсы, IPv6, только локальный. */
...
export default function Ports() {
  ...
  /*
   * Копии у порта нет: пара «адрес:порт» уникальна в пространстве, и второй
   * такой же сокет завести нельзя -- копия отличалась бы только именем.
   */
...
/*
 * Карточка порта -- та же стопка, что у сервера: имя сверху, дальше секции,
 * содержимое каждой -- одна таблица настроек (строка: директива, ⓘ, значение,
 * подстановки справа). Отдельной секции «свойства сокета» достаточно: ssl,
 * http2 и proxy_protocol -- хвост одной строки listen, и читаются они рядом.
 */
function PortForm({
  scope,
  id,

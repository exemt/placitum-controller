/**
 * Профили инспектора vlai: классификатор серьёзности, вердикт всегда score
 * либо allow. Профиль поэтому короткий -- режим, поведение при полной
 * очереди (overload) и две стороны канала действий: чьи просьбы слушать
 * (сигналы ранних волн) и что говорить соседям самому (правила).
 *
 * Блоки канала -- общие компоненты (signals-block, outcomes-block): разница
 * с контрактом и счётчиком -- пропсами. Особенности vlai две: триггер
 * «перегрузка» у инициаторов -- запрос снят на входе из-за полной очереди,
 * и профиль вправе рассказать об этом соседям либо внести клиента в набор;
 * субъект записи только адрес -- справочника гео у процесса нет.
 */
...
function VlaiProfileForm({
   ...
   * profile= читает именно его. Полоса над телом говорит, что профиль
   * разошёлся с поставкой, кнопка рядом возвращает его назад.
   */
      ...
      >
        ...
        >
          <SettingsTable aside={false}>
            ...
            />
            {/*
              Полная очередь: стоять до дедлайна или ответить сразу. Судьбу
              такого запроса называет маршрут -- «Сбои инспекции» на странице
              настроек, класс «инспектор не смог», -- а профиль решает только,
              ждать ли. Глубина очереди остаётся свойством процесса
              (inspector.conf в каталоге).
            */}
            <Pick

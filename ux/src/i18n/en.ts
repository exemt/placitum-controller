...
export const en: DeepString<typeof ru> = {
  ...
  incidentsPage: {
    ...
    query: {
      ...
      run: "search",
      revert: "restore the applied query",
      pending: "The query was edited but not applied. Press Enter in the line to search, Esc to restore.",
      moreErrors: "and {count} more",
      keys: "↑↓ select · Tab insert · Enter search · Esc close",
      keysPlain: "Enter search · Esc close · Ctrl+Space for suggestions",
      top: "most frequent in this window",
      topLoading: "counting…",
      stage: {
        field: "field",
        op: "comparison",
      ...
      op: {
        eq: "matches the whole value",
        like: "contains, case-insensitive",
        in: "any value from a list",
        between: "from one date to another",
        exists: "the key is present, any value",
        and: "add a condition",
      },
      list: {
        more: "another value",
      ...
      hint: {
        time: "A preset window or two dates: time in [\"2026-09-16 10:00\", \"2026-09-17 12:00\"].",
        verdict: "How the check ended.",
        ip: "An address or subnet: 203.0.113.7, 10.0.0.0/8, 2001:db8::/32.",
        host: "The whole Host header: shop.example.",
        uri: "Part of the path or query string, case-insensitive: \"/api\".",
        status: "Three digits: 403.",
        method: "Request method.",
        phase: "The phase the record was made in.",
        server: "Server name from the configuration.",
        route: "A route is a server and a location. Quote names that contain a space.",
        inspector: "An inspector that looked at the request.",
        user: "A source:login pair, such as corp:alice. A bare login searches every source.",
        marker: "The whole operator marker: bot-farm.",
        country: "Two ISO letters: ru, de, us.",
        asn: "Autonomous system number: AS9009.",
        ray: "The whole ray. With a ray, other conditions are ignored.",
        body: "Part of the request body, case-insensitive.",
        header: "Header name: header.user-agent. Without a value it means the header is present.",
        param: "Parameter name: param.id. Without a value it means the parameter is present.",
        pairValue: "The whole value. Quote spaces and punctuation.",
      },
      error: {
        unknownField: "There is no field “{field}”. Press Ctrl+Space for the list of fields.",
        unknownFieldHint: "There is no field “{field}”. Did you mean {hint}?",
        expectedField: "A field name goes here.",
        expectedOp: "{field} needs a comparison: {ops}.",
        expectedValue: "A value goes after the comparison.",
        expectedList: "in needs a list: {field} in [a, b].",
        listNeedsIn: "Write a list with in: {field} in [a, b].",
        unclosedList: "The list is not closed. Add ].",
        emptyList: "The list has no values.",
        missingComma: "Separate list values with a comma.",
        unclosedString: "The quote is not closed.",
        missingAnd: "Join conditions with and.",
        extraAnd: "Extra and.",
        danglingAnd: "A condition goes after and.",
        orUnsupported: "or is not supported: conditions are joined with and only. For several values of one field use in [a, b].",
        notUnsupported: "not is not supported: search only matches.",
        parensUnsupported: "Parentheses are not supported: write conditions in a row, joined with and.",
        opUnsupported: "There is no {op} comparison. Use =, ~ or in.",
        opWrong: "{field} only takes {ops}.",
        duplicate: "{field} is already in the query. For several values use in, if the field takes a list.",
        onePair: "A query can have one {field}.name condition.",
        pairName: "{field}. needs a name: {field}.name.",
        emptyValue: "Empty value.",
        badPreset: "There is no “{value}” window. Available: {values}.",
        timePair: "A date window takes two dates: time in [\"2026-09-16 10:00\", \"2026-09-17 12:00\"].",
        badStamp: "“{value}” is not a date. Write it like this: 2026-09-17 10:00.",
        badEnum: "{field} is one of: {values}.",
        badIp: "“{value}” is not an address or subnet. Example: 203.0.113.7 or 10.0.0.0/8.",
        badServer: "Server names have no spaces or commas. For several servers use server in [a, b].",
        badRoute: "There is no route “{value}”. Pick one from the suggestions.",
        badStatus: "A response code is three digits: 403.",
        badCountry: "A country is two ISO letters: ru.",
        badAsn: "An ASN is a number: AS9009.",
        tooLong: "{field}: up to 256 characters.",
      },
      saved: {
        title: "saved filters",
        name: "filter name",
        save: "save",
        update: "update",
        invalid: "Fix the query in the line first.",
        list: "saved",
        none: "Nothing here yet. Name the query above: it will appear here and stay in this browser.",
        recent: "recent",
        remove: "delete filter",
      },
      help: {
        title: "how to write a query",
        eq: "whole-value match, for almost every field",
        like: "part of the string, case-insensitive; uri and body only",
        in: "any value from a list: ip, server, route; for time, from one date to another",
        pair: "the header or parameter is present; with a value: header.name = value",
        pairSample: "header.name",
        and: "conditions are joined with and only; there is no or, not or parentheses",
        keys: "Ctrl+Space for suggestions · Tab to insert · Enter to search · Esc to restore",
        examples: "examples",
      },
    },
    card: {
  ...
  actionProfiles: {
    ...
    condName: "Name",
    condNameHint: "For example from_ads. A rule picks the condition by this name in «When»",
    ...
    condNameReserved: "rule-N names are taken: the panel names a rule's «When» so in the built profile",
    condFixName: "fix the condition name",
    condNeedName: "name the condition",
    condNeedValue: "set the value",
    ...
    when: "When",
    whenAlways: "always",
    whenConds: "by conditions",
    whenHint: "Always, by the profile's conditions or when the inspector is overloaded.",
    whenGroupsHint: "Inside a group — AND: every condition has to hold. Between groups — OR: one group is enough.",
    whenNoConditions:
      "The profile has no conditions yet. Add them in the «Conditions» section or right here: «new condition…» in a row's list.",
    whenNot: "not {name}",
    whenAnd: "AND",
    whenOr: "OR",
    whenIs: "if",
    whenIsNot: "if not",
    whenSign: "If or if not",
    whenPick: "pick a condition",
    whenGone: "not in the profile",
    whenNewCondition: "+ new condition…",
    whenRemove: "Remove from the group",
    whenAddAndHint: "Add a condition to this group: all of them have to hold",
    whenAddOrHint: "Add a group: any of the groups is enough",
    sectionRules: "Rules",
    rulesHint:
      "Signals to neighbours, the route event and points on the route — on every request of the profile or by conditions. No terminal rules: every row fires, in its order.",
    ...
    listCodeHint: "[A-Z][A-Z0-9_]*. Reason on the list entry; empty — ACTION_LIST",
    addRule: "Add a rule",
  ...
  cookieProfiles: {
    ...
    cookieAlert:
      "A cookie is a name and a value. The value is one of three kinds: a unique number (uid), different for every client; a concrete value, the same for everyone who gets the cookie; or taken from the request — an argument, a header or a field the client came with. Rules compare that value: one of the values typed in, none of them, in a list, not in a list. A marker gets it as `{value}`, and the value of any other cookie of the profile as `{cookie name}`: `client_{uid}_{value}`. A list write puts the value as «cookie value». Only [A-Za-z0-9_-] survive in the value; the rest becomes an underscore.\n\nThe inspector adds a signature itself: in the browser the cookie reads `value.time.signature`. The signature stops a cookie typed by hand in the console; the key belongs to the process (WAF_COOKIE_SECRET), the same for every copy; with no key the inspector answers error. Renewal lives in the signed part and has to be shorter than the lifetime.\n\nSecure, HttpOnly and SameSite are set by the route (waf_cookie_defaults). The module sets no domain — the cookie is bound to the server name.",
    path: "Path",
    ...
    value: "Value",
    valueSource: "Value",
    valueSources: { uid: "unique number (uid)", const: "concrete", request: "from the request" },
    valueSourceHints: {
      uid: "A random number of its own for every client: the client is recognised and listed by it",
      const: "The same for everyone who gets the cookie: marks those who passed this route",
      request: "An argument, a header or a request field the client came with",
    },
    valueFrom: "Request variable",
    valueFromHint: "For example $arg_utm_source — the referral source, or $http_x_client — the API client name",
    valueFallback: "When the request has none",
    valueFallbackHint: "[A-Za-z0-9_-]. Empty — such a client gets no cookie",
    valueFallbackShort: "else {text}",
    valueConst: "Value",
    valueConstHint: "[A-Za-z0-9_-], up to 128 characters",
    valueRandom: "Number length",
    valueRandomHint: "Bytes, 1..32: 8 bytes — 16 characters in the cookie",
    valueRandomShort: "+ number {n} bytes",
    valueUidShort: "uid, {n} bytes",
    valueMaxLen: "Value limit",
    valueMaxLenHint: "1..128, longer is cut",
    valueEmpty: "No value — the cookie will not save: pick a variable or type the value.",
    valueNeedRandom: "The number length is 1 to 32 bytes.",
    sectionRules: "Rules",
    ...
    overloadHint: "The inspector's queue reached the threshold: such a row has no cookie, phase or path",
    phase: "Phase",
    phaseHint: "Where the rule runs; empty — both",
    phaseBoth: "both",
    phases: { request: "request", response: "response" },
    status: "Response codes",
    statusHint: "A code and Enter. Empty — any code",
    statusBad: "Code «{code}»: three digits, 100–599",
    pathPrefix: "Path starts with",
    pathPrefixHint: "Empty — any path",
    pathShort: "path {path}",
    suffixesShort: "by path ending",
    methods: "Methods",
    methodsHint: "GET, POST… Empty — any method",
    methodsBad: "Method «{method}»: Latin letters only",
    stateAbsentHint: "\"No cookie\" together with issuing is the first touch: only a client without one gets it",
    when: "When",
    to: "To",
    toHint: "Self — issue or drop the cookie; or a receiving inspector, the route event, score, a list write",
    ...
    listOp: "Operation",
    listOpHint: "Add to the list or remove from it; removal takes no ttl",
    listCookie: "Which cookie",
    listCookieHint: "Empty — the rule's cookie",
    code: "Reason",
    codeHint: "[A-Z][A-Z0-9_]*. Empty — no reason",
    listCodeHint: "[A-Z][A-Z0-9_]*. The reason recorded in the list; empty — COOKIE_LIST",
    phasesAt: { request: "on request", response: "on response" },
    sectionWhenHint: "Which cookie, what the client presented and on which request",
    ...
    triggerHint: "What the client presented: whether the cookie is there and whether it is ours",
    tags: "Values",
    tagsHint: "Type and press Enter; several are fine",
    label: "Value",
    valueModes: {
      any: "any",
      one_of: "one of",
    ...
    valueModeHints: {
      any: "The cookie value is not checked",
      one_of: "Matches one of the values typed on the right",
      none_of: "Matches none of the values typed on the right",
      in: "Is in a list from «Data»",
      not_in: "Is not in a list from «Data»",
    },
    labelShort: "value: {tags}",
    labelNotShort: "value not: {tags}",
    listedList: "List",
    listedListHint: "A string list: dynamic or static",
    listedNone: "no string lists",
    listedPick: "pick a list",
    listedShort: "value in list {list}",
    notListedShort: "value not in list {list}",
    tagsBad: "Value «{tag}»: Latin letters, digits, _ and - only",
    toSelf: "own cookie",
    selfHint: "Issue or drop the cookie from the «Cookie» field",
    needTags: "type at least one value",
    needTagsFix: "fix value «{tag}»",
    needListed: "pick the list to check",
    needPathSlash: "the path starts with /",
    needMethodFix: "fix method «{method}»",
    needStatusFix: "fix code «{code}»",
    ...
    markerInsert: "Insert:",
    markerSlotValue: "value of {name}",
    markerSlotCookie: "value of {name}",
  ...
  setup: {
    ...
    title: "Setup wizard",
    blurb:
      "Four steps to a working server: the address, the virtual server, the protected nodes, the wiring. Nothing changes in the space until Create on the last step.",
    steps: {
      port: "Address",
      server: "Server",
      upstream: "Where to",
    ...
    step: "Step {n} of 4",
    loading: "Reading the catalogs…",
    portAsk: "Which address takes the traffic?",
    mode: "What we take",
    modeHttps: "HTTPS",
    modeHttpsHint: "0.0.0.0:443, ssl and http2. The server will need a certificate.",
    modeHttp: "HTTP",
    modeHttpHint: "0.0.0.0:80, no TLS.",
    modeCustom: "Custom address",
    modeCustomHint: "For a node behind a balancer or a non-standard port.",
    redirect: "Also listen on 80 and answer with a redirect",
    redirectHint:
      "The wizard adds a second virtual server with the same hostnames: its root answers 301 to https://$host$request_uri.",
    portHint: "1–65535.",
    hostHeaderHint: "The Host header for the protected server. Empty — whatever arrived.",
    portName: "Port name",
    portNameHint: "The name in the address catalog, not a server_name.",
    portReuse: "{listen} is already in the catalog — we take «{name}» instead of a second one.",
    portNew: "No such address in the catalog — we add it.",
    serverAsk: "Which hostnames does the server answer for?",
    ...
    addPeer: "+ node",
    dropPeer: "remove node",
    peersEmpty: "Add at least one node: host and port.",
    poolName: "Pool name",
    poolNameHint: "The name of the upstream {} block; the server root points at it.",
    poolReuse: "These nodes are already a pool — we take «{name}».",
    poolNew: "No pool with these nodes — we add one.",
    wireAsk: "Check the wiring and create.",
    defaultServer: "Default server on this address",
    defaultServerHint:
      "default_server: requests with an unknown Host land here. One such server per address.",
    defaultTaken: "{listen} already has a default server — we leave it alone.",
    plan: "What the wizard will do",
    planNew: "new",
    planReuse: "reused",
    task: {
    ...
    running: "Creating…",
    done: "Done. Now apply the configuration on the nodes — the button in the header.",
    doneCert:
      "A TLS server needs a server certificate: bind one on the «Virtual servers» page.",
    failed: "Stopped at «{step}»",
    failedHint: "Whatever was created stays in the space. Fix the cause and repeat.",
    retry: "Repeat",
    noSpace: "The wizard works inside a space — pick one in the header.",
  },

...
export const en: DeepString<typeof ru> = {
  ...
  cookieProfiles: {
    ...
    cookieAlert:
      "A cookie is a name and a value. The value comes from the request (an argument, a header, a variable), is a constant, or is not needed at all — then the cookie only tells clients apart. Rules compare that value: «one of», «none of», and a marker gets it as `{value}`. Only [A-Za-z0-9_-] survive in the value; the rest becomes an underscore.\n\nThe inspector adds a unique number and a signature itself: in the browser the cookie reads `google~1a2b3c4d.time.signature`. The number tells apart clients with the same value — lists hold the whole cookie. The signature stops a cookie typed by hand in the console; the key belongs to the process (WAF_COOKIE_SECRET), the same for every copy; with no key the inspector answers error. Renewal lives in the signed part and has to be shorter than the lifetime.\n\nSecure, HttpOnly and SameSite are set by the route (waf_cookie_defaults). The module sets no domain — the cookie is bound to the server name.",
    path: "Path",
    ...
    value: "Value",
    valueSource: "Value comes from",
    valueSources: { request: "the request", const: "a constant", none: "nowhere" },
    valueSourceHints: {
      request: "An argument, a header or a request variable the client came with",
      const: "The same for everyone who gets the cookie: marks those who passed this route",
      none: "The cookie only tells clients apart: a unique number and a signature",
    },
    ...
    valueFallbackHint: "[A-Za-z0-9_-]. Empty — the cookie goes out with no value, only a number",
    valueFallbackShort: "else {text}",
    valueConst: "Value",
    valueConstHint: "[A-Za-z0-9_-], up to 128 characters",
    valueRandom: "Unique number",
    valueRandomHint: "Bytes, 0..32. Tells apart clients with the same value; 0 — none",
    valueRandomShort: "number {n} bytes",
    valueMaxLen: "Value limit",
    valueMaxLenHint: "1..128, longer is cut",
    valueEmpty: "No value — the profile will not save: pick a variable or type a constant.",
    valueNeedRandom: "Without a value the cookie rests on its number: set at least 1 byte.",
    sectionRules: "Rules",
    ...
    tags: "Values",
    tagsHint: "Enter or a comma adds a value",
    label: "Value",
    labelHint: "What was written into the cookie when it was issued: google, direct",
    labelModes: { any: "any", in: "one of", not_in: "none of" },
    labelShort: "value: {tags}",
    labelNotShort: "value not: {tags}",
    listed: "List",
    listedHint: "Whether this cookie was written to a dynamic list",
    listedModes: { none: "do not check", in: "cookie in the list", not_in: "cookie not in the list" },
    listedList: "Which list",
    listedListHint: "A dynamic string list of the space",
    listedNone: "no dynamic string lists",
    listedPick: "pick a list",
    listedShort: "in list {list}",
    notListedShort: "not in list {list}",

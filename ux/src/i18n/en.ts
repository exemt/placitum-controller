...
export const en: DeepString<typeof ru> = {
  ...
  api: {
    ...
    // The tail after the colon lists the holders from detail.
  ...
  actionProfiles: {
    ...
    condName: "Name",
    condNameHint: "[A-Za-z0-9_][A-Za-z0-9._-]*. Rules pick the condition by this name; the audit shows it too",
    ...
    condModeAll: "all (AND)",
    condModeAny: "any (OR)",
    condModeHint: "How the rows combine: every one holds, or at least one does.",
    ...
    condAlert:
      "Rows are joined with AND (every one holds) or OR (at least one does) — the mode belongs to the whole condition. A row compares a value: `in` / `not in` with an active dataset of the space, `=` / `≠` with a text, byte for byte; or refers to another condition of the profile — «holds» / «fails», which gives AND inside OR and any depth. For a value with several occurrences (`all values`, `*`) `in` and `=` hold when any matches, `not in` and `≠` when none does.\n\nAn empty value means «not in the set»: no cookie — not among the trusted ones; a dataset the mirror has not received yet counts the same way. Headers, cookies and arguments come from the route's snapshot (waf_capture); without a snapshot $http_user_agent and the other standard fields are read from the declaration's vars. Conditions that refer to each other in a cycle are not saved.",
  ...
  cookieProfiles: {
    ...
    sectionProfile: "Profile",
    sectionProfileHint:
      "The inspector checks nothing and returns no verdict: it issues a cookie, drops it, writes its value into live datasets and tells the neighbours over the actions channel. What to do about that cookie is decided elsewhere — the local layer at the edge, neighbours' auto actions, the counter. Which paths this runs on is decided by the route the inspector is attached to with profile=.",
    sectionCookies: "Cookies",
    cookiesHint:
      "Declarations: what the cookie is, how long it lives, what fills its value and what signs it. A rule refers to a declaration by name — without one it will not save. Secure, HttpOnly and SameSite are not asked here: the route forces them (waf_cookie_defaults), and the module never sets a domain — the cookie stays host-only.",
    ...
    sectionConditions: "Conditions",
    conditionsHint:
      "Named conditions the rules refer to: \"if A\", \"unless A\". A condition is a set of `value · comparison · dataset or text` clauses joined by AND or OR; a clause can also be another condition (true / false). Values are the path, method, host, address and request fields from the message, plus headers, cookies and args from the exchange; datasets are the active lists of this space only.",
    condInUse: "Rules or other conditions ({n}) refer to \"{name}\" — detach them first.",
    sectionRules: "Rules",
    rulesHint:
      "When to issue a cookie, when to drop it and what to tell the neighbours about it. There are no terminal rules: every matching row fires. The fight over the cookie itself is not settled by order — dropping beats issuing.",
    rulesEmpty: "No rules: the inspector stays silent.",
    addRule: "Add rule",
    editRule: "Edit rule",
    operation: "Operation",
    operationHint: "Issue the cookie, drop it, or neither — just tell",
    opNone: "nothing",
    opIssue: "issue",
    opDrop: "drop",
    opAdd: "add",
    ...
    state: "Cookie state",
    stateHint: "What the client presented; empty — any",
    ...
    when: "When",
    whenHint: "Always, or when a profile condition is true or false.",
    ...
    to: "To",
    toHint: "A receiving inspector, the route record, score, or a dataset write. Nobody is fine too — the rule will just issue the cookie.",
    toNobody: "tell nobody",
  ...
  outcomes: {
    ...
    outcomeTo: "To",
    /* Pseudo-addressee of the record verbs: the route's own log and archive. */
    toModule: "the route's record",
    /* Pseudo-addressee of points: the route's own phase sum, executed by the module. */
    toScore: "Points on the route",
    /* A points row in the table: the addressee is the route itself. */
    toRoute: "the route",
  ...
  captcha: {
    ...
    /* Provider summary in the row: one phrase instead of its fields. */
  ...
  ipProfiles: {
    lockedDelete: "The default profile is not deleted: the manifest does not build without it.",
    /* The dataset ttl in one phrase: a rule row is tight as it is. */
    ttlShort: "ttl {n}s",
    description: "Description",
    rules: "Rules",
    newTitle: "New profile",
  ...
  /*
   * Inter-inspector action vocabulary -- human names for verbs and axes.
   * Shared by both sides of the channel: the sender page (ip rules) and the
   * receiver's prior rules (captcha) call the same thing by the same words.
   * The vocabulary itself arrives from GET /api/actions; only labels live here.
   */
  actions: {
    verbs: {
      challenge: {
    ...
    /* Sections of "What to do": call mode (executed by the module) above requests to a neighbour. */
    group: {
      record: "The route's record",
      mode: "Inspector mode",
      ask: "Request to the inspector",
    },
    /* Record verbs: the side, and for the archive — objects, TTL and limit. */
    audit: {
      record: "Record",
      recordHint:
    ...
    /* Who it is about: this request, the address, the whole provider network, the session. */
    axes: {
    ...
    /* Counter change: direction in words, the panel sets the sign. */
    counter: {
      label: "About whom",
      bucket: "Bucket",
      bucketHint:
    ...
    /* Phase of the addressee's call for the control verbs. */
    phase: {
      label: "Call phase",
      hint:
    ...
    /* Points on the route: direction in words, magnitude without a sign. */
    score: {
      direction: "Add or remove",
      add: "add to the sum",
    ...
    /* Direction in words: nobody types the sign by hand. */
    direction: {
  ...
  config: {
    ...
    /*
     * The setup wizard is long gone; the only surviving label is the deny
     * page in the score editor. Form row labels live in field: the directive
     * name is the label.
     */
    op: {
      scorePage: "Deny page",
  ...
  placeholder: {
    body: "This section comes later — shell layout only for now.",
  },
  traffic: {
    title: "Traffic",
    blurb:
      "wrk hits HAProxy on the already-running stand; containers stay up. A case from tests/load fills the whole form, edit it after. The step number is an rps cap; actual rps is in the result; worker CPU is on the status page.",
    runnerHost: "wrk on the host",
    runnerHostHint: "Needs wrk in PATH. The usual path is the loadgen container.",
    runnerDocker: "wrk in Docker",
    run: "Load the stand",
    running: "Running…",
    disabled:
      "Generator is not connected. The usual path is the loadgen service in compose; fallback is deploy/loadgen/run-host.ps1.",
    failed: "wrk exited with an error. Below is what it managed to record.",
    cases: "E2E cases",
    casesHint:
      "tests/load/cases.mjs: one case is one route with a ready pipeline. A click fills the form: target, request, flags, steps.",
    casesError: "Case catalogue not read:",
    casesEmpty: "No cases. The catalogue is tests/load/cases.mjs; the generator re-reads it on its own.",
    caseSection: "Section",
    caseTitle: "Case",
    caseRoute: "Route",
    casePipeline: "Inspectors",
    casePipelineHint: "Calls × waves on the request phase",
    caseSteps: "Steps, rps",
    caseFixture: "The fixture creates the route and declarations:",
    caseOf: "case",
    caseOfHint: "Form was filled by the case; edits stay, step thresholds are its. Remove with the cross.",
    section: {
      load: "load",
      logic: "logic",
    },
    request: "Request",
    requestHint: "One template per run. Host goes as a header; wrk hits the stand HAProxy.",
    host: "Host",
    method: "Method",
    path: "Path",
    expect: "Expect code",
    expectHint: "A response with another code counts as unexpected.",
    headers: "Headers",
    headersHint: "One per line: Name: value. No Host here.",
    body: "Body",
    bodyHint: "As is. Size padding extends JSON and forms with a _pad field, anything else with a tail.",
    randomIp: "Client address",
    randomIpHint:
      "A new X-Forwarded-For per request — for per-address buckets, limits and lists. The stand server trusts the header from any address.",
    ipFixed: "fixed",
    ipPool: {
    ...
    ipPoolHint: {
      public: "Any IPv4 outside reserved ranges.",
      private: "10/8, 172.16/12, 192.168/16.",
      testnet: "203.0.113/24, 198.51.100/24, 192.0.2/24 — what usually sits in a blocklist.",
    },
    flags: "Flags",
    unique: "unique query",
    uniqueHint: "?_n=<thread>-<seq> per request: past caches and with distinct parameters.",
    bodiesSize: "Body size",
    bodiesSizeHint:
      "POST/PUT/PATCH only: the body stays, a tail pads it to the bucket. “as is” — no padding. A large bucket at a high step is bandwidth, not CPU.",
    bodyOrig: "as is",
    headersSize: "Header size",
    headersSizeHint: "X-Load-Pad is padded to the chosen size. “as is” — only the form headers.",
    argsSize: "Query size",
    argsSizeHint: "&_pad= is padded to the chosen size. Path parameters stay.",
    steps: "Steps",
    stepsHint: "In order, each its own wrk run. The number is the step rps cap.",
    rateField: "rps",
    seconds: "sec",
    addStep: "Step",
    ladder: "Ladder",
    ladderHint: "Replaces the steps: start × factor on every next one.",
    ladderStart: "start, rps",
    ladderFactor: "factor",
    ladderCount: "steps",
    ladderBuild: "Build",
    invalid:
      "Steps: 1–{steps}, rps 1–{rate}, duration 1–{sec} s, total at most {total} s. Host and path are required, headers are “Name: value”, response code 100–599.",
    plannedShort: "{n} requests planned",
    result: "Result",
    ceiling: "ceiling {rps} rps",
    ceilingNone: "no clean steps",
    ceilingHint: "The last clean step before the first dirty one.",
    planned: "Planned",
    reqs: "Requests",
    rps: "actual rps",
    unexpected: "Wrong code",
    sockets: "Socket errors",
    socketsHint:
      "wrk connect/read/write: the request either did not establish TCP or was cut off. Check HAProxy and nginx CPU.",
    timeouts: "wrk timeouts",
    connections: "Connections",
    codes: "Codes",
    waf: "Module, X-WAF-Debug",
    wafHint:
      "The debug header parsed on every response: verdict, fail= and how long the module waited for each inspector (bus RTT plus its work). If client p99 ≈ wait p99, the bottleneck is here; if the client is noticeably higher — body, proxy or network.",
    seen: "With header",
    wafWait: "Verdict wait",
    wafWaitHint: "The slowest inspector of the request",
    inspNone: "inspector none",
    fails: "Module failures",
    verdicts: "Verdicts and failures",
    ...
    bus: "NATS bus",
    busHint:
      "Snapshot after the run. pending on an inspector — it cannot keep up reading. pending on an edge — the module cannot keep up writing. A slow consumer during the run means a subscriber was dropped for lagging.",
    connection: "Service",
    replicas: "replicas",
    pending: "pending",
    ...
    msgs: "messages",
    log: "wrk log",
    logHint: "wrk output per step, last 8 KB.",
  },
  inherit: {
    tag: {
      http: "http",
  ...
  registry: {
    ...
    addInspector: "Add an inspector from the catalog…",
    allDeclared: "Every inspector in the catalog is already declared",

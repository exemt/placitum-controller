...
/*
 * Страница профилей капчи. Тот же каркас, что у калитки (auth.ts): адрес
 * страницы выбирают из локейшенов сервера, активный список -- из объявленных
 * наборов, страницу -- из раздела «Страницы».
 */
...
/*
 * Секрет внешнего провайдера -- store-объект: шифруется в браузере ключом
 * контура, как PEM сертификата; контроллер видит только конверт, открывает
 * captcha-http. Возвращает uuid, который уходит в secretStore профиля.
 */

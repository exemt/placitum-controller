...
/**
 * Копия маршрута под новым путём.
 *
 * Пара «сервер + признак + путь» уникальна, поэтому копия -- всегда с другим
 * путём: окно спрашивает его, а не имя. Образец берётся из строки: список
 * отдаёт маршрут целиком, вместе с оверлеями nginx и waf.
 */
...
const pathsPageSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(reorderLocationsThunk.fulfilled, (state, action) => {
      // Ответ -- пути одного сервера с новыми позициями; остальные серверы
      // не трогаем, но список держим в том же порядке, что отдаёт контроллер.

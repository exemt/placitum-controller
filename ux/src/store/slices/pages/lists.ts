 ...
 *
 * Текст лежит только в детали -- список отдаёт мету, -- поэтому копия сперва
 * читает образец, а потом заводит новый файл. Карточку она не трогает: копию
 * делают из строки, и открывать за неё панель незачем.
 */
...
/*
 * Отказ (409 `in_use`: файл в составе профилей) показывает полоса формы:

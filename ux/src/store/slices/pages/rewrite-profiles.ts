...
/*
 * Страница профилей инспектора rewrite. Каркас тот же, что у vlai
 * (vlai-profiles.ts); из справочников нужен один -- каталог ответов отказа:
 * его называет профиль, и опечатка в имени иначе всплыла бы ERROR-строкой
 * модуля на живом отказе.
 *
 * Реестра инспекторов здесь нет: инициаторов у rewrite не бывает, он только
 * принимает просьбы. Отправителей, которых нет в контуре, считает контроллер и
 * присылает их с карточкой (unknown_senders).
 */
...
/**
 * Копия профиля под новым именем. Образец перечитывается с сервера, а не
 * берётся из строки списка: список мог устареть.
 */
...
/**
 * Вернуть default к поставке. Карточка перечитывается загрузкой детали:
 * форма держит поля от `detail`, и половинчатое состояние разошлось бы.
 */

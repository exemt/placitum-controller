...
function buildTheme(mode: "dark" | "light") {
  ...
  return createTheme({
    ...
    zIndex: {
      appBar: 1000,
      modal: 4000,
      // Панели и диалоги подняты до 4000; подсказка обязана быть над ними,
      // иначе tooltip внутри Drawer уходит под его же полотно.
      tooltip: 4100,
    },
    components: {
      MuiCssBaseline: {
        styleOverrides: {
          /*
           * Цвета ячейки таблицы -- переменными.
           *
           * Ячейка (`components/data-table/FilterCell.tsx`) красится одним
           * постоянным правилом: их в таблице сотни, у набора инспекторов --
           * под две сотни на карточку, и разбирать `sx` по теме на каждую при
           * каждом рендере было ощутимо дороже самой таблицы. Постоянному
           * правилу тема на рендере не нужна, а переключается оно здесь.
           */
          ":root": {
            "--cell-paper": paper,
      ...
      MuiInputLabel: {
        ...
        styleOverrides: {
          root: {
            // Сдвиг неподнятого лейбла MUI считает под свои паддинги input;
            // с ужатыми до 6px (MuiOutlinedInput выше) подпись висит ниже
            // центра поля.
      ...
      MuiChip: {
        styleOverrides: {
          root: {
            ...
            /*
             * Значки по краям фишки -- зеркальны друг другу: ведущий отступает
             * от левого края, крестик -- от правого. Общее правило на оба
             * сажало крестик на 2px за правый край фишки, а между ним и
             * подписью оставляло 10px: фишка читалась косой.
             */
            "& .MuiChip-icon": {
              fontSize: 12,
              marginLeft: 4,

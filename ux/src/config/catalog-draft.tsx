/*
  Черновик каталогов http: списки, ответы отказа, обменник объектов, форматы логов.

  Ячейки этих таблиц раньше коммитили PUT по blur, и страница жила в двух
  режимах: директивы ждали «Сохранить», каталоги уезжали сразу -- «не
  разослано» вспыхивало до кнопки. Теперь правки копятся здесь, а «Сохранить»
  отдаёт серверу разницу теми же вызовами, которыми раньше коммитили ячейки.

  Успешная операция сразу впитывается в срез сервера (base): упади следующая
  -- в черновике остаётся ровно несохранённый остаток, и повторное
  «Сохранить» дошлёт его, не дублируя создания. При полном успехе всё
  перечитывается: uuid созданных, size и vars считает сервер.
*/
...
/** Префикс uuid несохранённой строки: настоящий выдаст POST на «Сохранить». */
const NEW_ID = "new:";

/** Строка ещё не была на сервере: серверных полей (url, size) у неё нет. */
...
/** Чего нет в строке набора: параметры будущего POST нового списка. */
interface ListSeed {
  type: DatasetType;
  copyFrom?: string;
}
...
/**
 * Разница строки набора со срезом сервера -- телом PUT. Пустой ttl не
 * отправляется и правкой не считается: сервер пустое значение игнорирует
 * (coalesce в update), сбросить срок этим PUT нельзя.
 */
...
/*
 * Отпечатки правимых полей: сравнение строк без серверных довесков (size,
 * vars, updated_at). Патчи копируют объекты с сохранением порядка ключей,
 * поэтому строковое сравнение JSON здесь честное.
 */
...
export interface CatalogDraft {
  loaded: boolean;
  /** Наборы kind=list: объявленные и запасные вместе, объявленность -- in_nginx. */
  lists: Dataset[];
  /** Страницы (kind=content): справочник для записей отказа, отсюда не правятся. */
  pages: Dataset[];
  deny: DenyResponseRow[];
  stores: BodyStoreRow[];
  formats: LogFormatRow[];
...
export function useHttpCatalogDraft(): CatalogDraft {
  ...
  const patchList = useCallback<CatalogDraft["patchList"]>(
    (uuid, patch) => {
      // Снятый с конфига несохранённый список не «запасной»: его просто не было.
  ...
  const addStore = useCallback<CatalogDraft["addStore"]>(() => {
    ...
    patchRows((cur) => ({
      ...
      stores: [
        ...cur.stores,
        // Адрес печатает контроллер из окружения: у несохранённого его ещё нет.
        {
          uuid,
          name: "hot",
          driver: "redis",
  ...
  const save = useCallback(async () => {
    ...
    try {
      // Удаления первыми: освобождают имена под переименования и создания.
      ...
      // Правки: одна строка -- один PUT со всеми её полями.
      for (const row of rows.datasets) {
        const prev = base.datasets.find((r) => r.uuid === row.uuid);
      ...
      // Создания последними: имя могло освободиться удалением выше.

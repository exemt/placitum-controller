 ...
 *
 * Таблица показывает, а не правит (settings-table.md, правило 15, случай
 * четвёртый): в ячейке -- состояние словами («весь», «первые 30k», «как
 * снимок»), скобка уровня и пометки, а правится объект целиком -- окном, которое
 * открывает клик по строке. Микрополя в ячейках («30k / 2k» без подписей,
 * галочка при слове «весь») были главным источником вопросов: две величины
 * записи, три состояния галочки и placeholder, читавшийся значением.
 *

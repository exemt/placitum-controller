...
export type LoadHeader = { name: string; value: string };
...
export type LoadFlags = {
  /** Пул случайного X-Forwarded-For; пусто -- адрес фиксированный. */
  random_ip: string;
  /** Уникальный query на каждый запрос. */
  unique: boolean;
};

export type LoadSizes = { body: string[]; headers: string[]; args: string[] };

export type LoadStepPlan = { rate: number; duration_s: number };
...
export const LOAD_SIZE_TAGS = ["orig", "1k", "8k", "64k", "256k", "1m"] as const;
export const LOAD_META_TAGS = ["orig", "1k", "8k", "64k", "256k"] as const;
export const LOAD_IP_POOLS = ["public", "private", "testnet"] as const;
export const LOAD_METHODS = ["GET", "HEAD", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"] as const;

/** Что модуль написал в X-WAF-Debug за ступень. */
export type LoadWaf = {
  seen: number;
  ...
  none: number;
  wait: LoadStat | null;
  inspectors: (LoadStat & { name: string })[];
};

export type LoadStepResult = {
  step: LoadStepPlan;
  seconds: number | null;
  reqs: number;
  rps: number | null;
...
export type LoadClean = {
  unexpected_pct: number | null;
  fails: number | null;
  sockets: number | null;
  p99_ms: number | null;
};
...
export type LoadBus = {
  ...
  slow_consumers_delta: number | null;
  in_msgs_delta: number | null;
  out_msgs_delta: number | null;
  connections: LoadBusConn[];
};

export type LoadRun = {
  status: "disabled" | "idle" | "running" | "done" | "error";
  runner?: "host" | "docker";
  target?: string;
  limits?: LoadLimits;
  plan?: LoadPlan;
  ...
  log?: string;
  totals?: LoadTotals;
  by_step?: LoadStepResult[];
  /** Последняя чистая ступень до первой грязной, rps; null -- чистых нет. */
  ceiling?: number | null;
  clean?: LoadClean;
  bus?: LoadBus | null;
};

/** Кейс из tests/load/cases.mjs, каким его отдаёт генератор. */
export type LoadCase = {
  ...
  route: {
    ...
    position?: number;
    waf?: {
      requestInspectors?: { name: string; wave?: number }[] | "none";
      responseInspectors?: { name: string; wave?: number }[] | "none";
      deadlineMs?: number;
      exception?: string[];
    };
  };
  declare?: Record<string, { process?: string; profile?: string }>;
  traffic: {
    host: string;
    method: string;
    path: string;
  ...
  clean?: Partial<LoadClean>;
};

export type LoadCasesReply = {
  cases: LoadCase[];
  error: string | null;
  path?: string;
};
...
export async function fetchLoad(): Promise<LoadRun> {
  const res = await fetch("/api/load");
  if (res.status === 503) {
    const body = (await res.json()) as LoadRun;
    return { ...body, status: "disabled" };
  }
  return parseJson<LoadRun>(res, "/api/load");
}

export async function startLoad(plan: LoadPlan): Promise<LoadRun> {
  const res = await fetch("/api/load", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(plan),
  });
  if (res.status === 409 || res.status === 202 || res.ok) {
    return (await res.json()) as LoadRun;
  }
  return parseJson<LoadRun>(res, "/api/load");
}

export async function fetchLoadCases(): Promise<LoadCasesReply> {
  const res = await fetch("/api/load/cases");
  if (res.status === 503) {
    return { cases: [], error: "loadgen_unreachable" };
  }
  return parseJson<LoadCasesReply>(res, "/api/load/cases");
}

/*
 * Разбор, проверка и печать документа источника входа.
 *
 * Правила здесь -- те же, что в Validate() инспектора
 * (inspectors/auth/internal/config/source.go). Их два экземпляра, и это
 * осознанно: контроллер обязан отказать оператору в форме, а инспектор --
 * не подняться на битом источнике, даже если тот приехал мимо контроллера.
 * Разъезд ловится прогоном tests/auth: поколение, которое контроллер выпустил,
 * а инспектор отверг, видно в пульсе как apply_failed.
 *
 * Печать -- в YAML, а не в JSON: манифест несёт текст source.yaml, и на
 * стороне инспектора его читает тот же загрузчик, что и файл с диска. Третьей
 * реализации разбора не появляется.
 */
...
/*
 * normalizeSourceDoc приводит присланное к документу и заполняет умолчания.
 * Всё, чего оператор не назвал, получает то же значение, что и в
 * sourceDefaults() инспектора -- иначе источник, созданный в UX, вёл бы себя
 * не так, как тот же источник из файла.
 */
export function normalizeSourceDoc(raw: unknown): AuthSourceDoc {
  ...
  return {
    ...
    session: {
      /*
       * Пустое имя -- умолчание инспектора: waf_sid_<имя источника>. Источник
       * и есть пространство сессий, куки двух источников не совпадают --
       * это держит контроллер на записи и реестр инспектора на загрузке.
       */
    ...
    ticket: {
      /* Пусто -- waf_lgn_<имя источника>, как у сессии. */
...
/*
 * Проверка своей формы входа. Она не про вёрстку -- вёрстка дело оператора, --
 * а про три вещи, без которых форма не сможет никого впустить:
 *
 *   - POST-форма: калитка принимает только её;
 *   - поле csrf со значением {{.Nonce}}: билет одноразовый, и совпадение
 *     nonce из cookie с полем формы -- то, что проверяется до провайдеров;
 *   - хотя бы одно поле ввода, иначе вводить нечего.
 *
 * Без этого страница выглядит рабочей и молча не пускает -- ровно тот случай,
 * который обязан ловиться на записи, а не на живом контуре.
 */
...
export function validateSourceDoc(doc: AuthSourceDoc): void {
  /*
   * Форма обязательна: источник без формы не способен никого впустить.
   * Прежние профили-спутники -- профили с пустыми redirect_methods на живом
   * источнике, а не источник без формы.
   *
   * У внешних провайдеров формы нет по построению: login.uri -- страница
   * входа приложения, куда уводить навигацию без сессии; пусто -- 401 всем.
   */
  ...
  if (doc.list.sessions !== "") {
    ...
    /*
     * Окно на доезд записи, а не второй срок сессии: потолок тот же, что в
     * Validate() инспектора. Ноль -- умолчание (15 с), не запрет окна.
     */
...
/* Те же правила, что validateJWT() инспектора, плюс одно своё: секрет HMAC в
 * документе не живёт -- только имя переменной окружения. */
...
function validateCode(code: AuthCodeProvider | null, from: string): void {
  ...
  if (code.kind === "totp") {
     ...
     * источника (identity.from), секрет -- из набора пользователей по логину.
     */
    if (from === "") {
...
/*
 * Пароль служебной записи -- ровно одним способом. Оба сразу означали бы, что
 * оператор не знает, какой из них работает, а на ноде победит один.
 */
...
/*
 * renderSourceYaml печатает документ ровно в ту форму, которую читает
 * загрузчик инспектора. Поля, которых оператор не трогал, печатаются тоже:
 * источник в манифесте -- полный снимок, а не дельта к умолчаниям, и читать
...
export function renderSourceYaml(name: string, doc: AuthSourceDoc): string {
  ...
  /*
   * Имя куки = источник = пространство сессий. Пустое имя не печатается:
   * умолчание инспектора -- на источник, и снимок показывает его
   * комментарием, чтобы при разборе инцидента не ходить в исходники.
   */
  out.push("session:");
  ...
  if (doc.providers.jwt !== null) {
    ...
    /* Публичный ключ едет файлом рядом с source.yaml, как login.html. */
    if (j.verify.key !== "") {
...
/*
 * Запись пользователя в списке: login:bcrypt[:группы[:store-uuid секрета TOTP]].
 *
 * Двоеточие как разделитель безопасно: в логине оно запрещено, а bcrypt-хеш
 * состоит из $, точек, слэшей и букв с цифрами. Группы -- через запятую.
 *
 * Строка, начинающаяся с #, -- выключенная запись: список её несёт, файл
 * пользователей её не получает. Так же ведут себя комментарии в любом списке.
 */
...
/*
 * renderUsersYaml печатает пользователей из строк списка. Секрет TOTP сюда не
 * попадает никогда: в файл едет ссылка на store-объект, а открывает её процесс
 * на ноде ключом контура.
 */

...
/**
 * Собрать действующие настройки пути. Идёт сверху вниз, каждый уровень
 * затирает одноимённые ключи родителя. Массивы не конкатенируются: три проверки
 * на сервере и одна на пути -- на пути остаётся одна.
 *
 * `undefined` в значении ключа после слияния быть не должно: отсутствующий
 * ключ просто не копируется.
 */
...
/**
 * Ядро nginx, которое сервер может переопределить. Остальное http-only
 * (sendfile, keepalive) на server {} не спускается.
 */
...
/**
 * Ядро nginx, которое путь берёт из http {}, минуя сервер, если сервер
 * ключ не затёр. add_header и пресет прокси на server {} нет.
 */
...
/** Ядро nginx, которое путь берёт с server {}, если ключ там есть. */
...
/** Один ключ, который ребёнок получит сверху, если сам его не запишет. */
...
/**
 * Что сервер видит из http {}: весь waf и пересечение nginx.
 * sendfile и шина сюда не входят -- они не наследуются.
 */
/**
 * Реестр -- объявление контура, а не настройка маршрута: сервер его не
 * «наследует», он им пользуется. Показывать `inspectors` среди унаследованных
 * ключей значило бы обещать, что сервер получил оттуда набор инспекторов, --
 * а набора в http нет вовсе.
 */
...
/**
 * Что путь видит из http {} и server {}: замена ключа, как у resolveRoute.
 * Позже записанный уровень побеждает, в том числе пустым массивом.
 */

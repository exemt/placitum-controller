...
/**
 * `kind` в join неоднозначен: он есть и у привязки (назначение), и у самого
 * сертификата (тип). Назначение остаётся под именем `kind`, тип приезжает
 * под алиасом `cert_kind` -- иначе один перезаписывал бы другой.
 */
...
/**
 * Метаданные из crypto-сервиса, не от браузера -- см. docs/crypto-service.md.
 * `blob` сюда не попадает: repo знает только про ссылки на store_objects.
 */
export interface CertificateInsert {
  ...
  certStoreId: string;
  /** Только у `type: "server"`; check-констрейнт в базе это подтверждает. */
  keyStoreId?: string;
  chainStoreId?: string;
  sans: string[];
  notBefore?: Date;
...
/** Разбор CRL crypto-сервисом -- то, что кладётся на строку client_ca. */
...
export class CertificateRepo {
  ...
  /**
   * Замена списка отзыва. store-объекты неизменяемы, поэтому здесь всегда
   * новый `storeId`; старый блоб остаётся в store_objects и умирает вместе
   * со своим scope.
   */
  async setCrl(
    id: string,
  ...
  /** Снять список отзыва: nginx перестаёт печатать `ssl_crl` для этого CA. */
  ...
  async bind(input: {
    ...
    // Серверную пару нельзя повесить в ssl_client_certificate, а корень
    // mTLS -- в ssl_certificate: у первого не тот смысл, у второго нет
    // приватного ключа и nginx откажется стартовать.

...
/*
 * Размер nginx (`192`, `8k`): в документе строка, но голое число -- тот же
 * размер, и в jsonb стенда оно лежит числом. Строгий `asStr` ронял на нём
 * и запись карточки, и превью, хотя компилятор печатает оба одинаково.
 */
...
/*
  Путь уезжает в конфиг как есть, поэтому проверяется здесь: абсолютный, без
  пробелов и без символов, которыми закрывается директива. Хвост `level1..3`
  nginx допускает, но панель его не предлагает -- каталог один, и раскладывать
  временные файлы по подкаталогам на нём незачем.
*/
...
function parseBus(
  ...
  /*
   * `urls`, `user`, `pass` и `token` присланными не сохраняются: адрес шины и
   * доступ к ней -- развёртывание, а не документ пространства (см. InfraUrls).
   * Старый клиент, который их всё ещё шлёт, не получает ошибку -- поля просто
   * не доезжают, и при первом же сохранении уходят из документа.
   */
...
function parseVars(
  ...
  for (const item of value) {
    ...
    // Тот же набор символов, что у модуля: имя едет ключом JSON и токеном лога.
    if (!VAR_NAME_RE.test(name)) {
      return fail("invalid_waf_var_name");
    }
    // Стандартное поле едет само; второй источник под тем же именем -- nginx -t.
...
/**
 * `vars=` объявления: имена полей либо `all`. Есть ли такое поле -- знает
 * компилятор, у которого перед глазами и стандартный набор, и `waf_var`
 * пространства; здесь только форма.
 */
...
function parseInspectorDecl(value: unknown): InspectorDecl | ParseFail {
  ...
  // `weight` старых документов не читается вовсе: веса больше нет нигде.
  ...
  // Ссылка на процесс -- то же пространство имён, что и ключи графа.
...
/**
 * `if <значение> in|not in <набор>`. Условие принимают три директивы, поэтому
 * разбор один на всех: строка без значения или без набора -- не «условие по
 * умолчанию», а недописанное условие, и такую директиву нода не примет.
 */
...
function parseInspectorRef(value: unknown): InspectorRef | ParseFail {
  ...
  // `weight` старых вызовов молча не переносится: множителя у модуля нет, а
  // строка с ним роняет nginx -t.
...
function parseInspectorList(
  ...
  for (const item of value) {
    ...
    // `resume=` на фазе запроса роняет `nginx -t`: запрос продолжение
    // выдаёт, а не потребляет. Ключ отвергается здесь, а не молча теряется
    // при печати -- иначе строка выглядела бы настроенной, не будучи ею.
    if (row.resume !== undefined && field === "request_inspectors") {
      return fail("resume_on_request");
    }
    // Кадры инспектируются каждый сам по себе и транзакцию рукопожатия не
    // продолжают: `resume=` на них -- `nginx -t`. Потребляет только ответ.
    if (row.resume !== undefined && field === "frame_inspectors") {
      return fail("resume_on_frame");
    }
    // Зеркально: `keep=` -- у той фазы, что состояние выдаёт, и только у неё.
...
/**
 * `access_log`. Три формы на входе: строка (хвост директивы целиком, как
 * лежало в jsonb раньше и как оператор вводит одной строкой), `"off"` и
 * массив объектов. На выходе всегда одна из двух форм модели.
 */

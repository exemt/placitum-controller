...
/*
 * Имя узла -- и в рукопожатии, и в заголовке -- печатается в конфиг как есть,
 * поэтому в нём нет места пробелам, кавычкам и точке с запятой: строка
 * директивы иначе закроется раньше времени и утянет за собой остаток файла.
 * Переменные nginx ($host) здесь тоже не нужны: пул на то и пул, чтобы не
 * зависеть от того, каким именем пришли к нам.
 */

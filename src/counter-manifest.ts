...
/**
 * Поколение профилей счётчика. Ключ и канон хеша те же, что у auth, captcha,
 * json и modsec -- docs/inspector-config-distribution.md. Общая секция едет
 * псевдопрофилем `_shared` с единственным файлом counters.yaml: формат
 * манифеста этого не замечает, а загрузчик инспектора читает `_shared` сам и
 * профилем его не считает.
 *
...
/* Подчёркивание в начале разрешено намеренно: `_probe` и `_shared` --
 * зарезервированные имена, и держать их под управлением контроллера так же
 * законно, как любой другой профиль. */
...
export async function buildCounterManifest(
  ...
  /*
   * Общая секция обязана существовать и быть исправной: каталог без
   * counters.yaml инспектор не поднимет вовсе, и это будет apply_failed без
   * единого понятного слова.
   */
  let shared;
  ...
  for (const row of rows) {
    ...
    try {
      ...
      // Связность проверяется на send целиком: правка общей секции могла
      // осиротить ссылку профиля, сохранённого раньше неё.

...
/**
 * Необязательное целое. `null` и пустая строка значат то же, что `undefined`:
 * «не задано». Панель шлёт одно тело на создание и на правку, и пустое числовое
 * поле уезжает в нём как null (ux/src/pages/Paths.tsx). Отвергать его значило бы,
 * что ни один путь не создаётся, пока оператор не впишет код ответа, — а код
 * нужен только handler=return. Очистку поля на правке это не ломает: там null
 * отличим от «не присылали» и разбирается до вызова (parseLocationPatch).
 */
...
/**
 * Цель `error_page` у handler=return: именованный путь (`@waf_deny`) или URI
 * (`/50x.html`). Форма проверяется здесь, а не только в панели: в строку
 * уходит директива nginx, и пробел или `;` в ней роняют `nginx -t` уже на
 * ноде -- то есть после раскатки.
 */

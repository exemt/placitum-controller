/*
 * Документ профиля контракта: нормализация, проверка и печать в profile.yaml.
 *
 ...
 * проверка здесь ровно та же и в том же порядке.
 *
 * Умолчания фаз разные намеренно: цена ошибки несимметрична. Отказ на запросе
 * не пускает мусор в приложение, отказ на ответе ломает витрину после того, как
 * приложение уже отработало.
 */
...
/*
 * Просьбу целиком проверяет общий валидатор канала (model/action-ask.ts): её
 * форма одна на всех отправителей, и своя копия проверок означала бы, что
 * расширение канала надо не забыть ещё и здесь.
 */
const METHOD_RE = /^[A-Z]+$/;
const UUID_RE =
...
/**
 * Умолчания правил. Разные у фаз по той же причине, по которой различаются
 * действия: отказ на запросе не пускает мусор, отказ на ответе ломает витрину
 * после того, как приложение отработало.
 */
...
/*
 * Направления кадра повторяют пару запрос/ответ: сторона клиента гейтит --
 * контракт как белый список сообщений, включая тип, которого в нём нет, --
 * выдача приложения скорит.
 */
...
/** Запись каталога для отказа на кадре: type=websocket, заводится миграцией. */
const FRAME_DENY_RESPONSE = "ws_policy";

/**
 * rules читает таблицу политик фазы -- и новую форму, и ту, что писали до неё.
 *
 * Старая форма: восемь ключей `on_*` со строкой действия и один `score` на всю
 * фазу. Читать её здесь обязательно: документы уже лежат в базе, и профиль,
 * записанный неделю назад, не должен превращаться в набор умолчаний из-за
 * того, что форма стала точнее.
 */
function rules(
  ...
  for (const outcome of outcomes) {
    ...
    // Старая форма: on_<исход> плюс общий score фазы.
 ...
 * формы. Нормализация на чтении, а не только на записи: строка, записанная до
 * появления поля, обязана прочитаться с его умолчанием, а не с undefined.
 */
...
/* Секция направления кадра: проверки, политики, запись отказа, инициаторы. */
...
/*
 * validateDoc отвергает то, что инспектор отвергнет на ноде. Смысл проверки
 * здесь -- поймать ошибку оператора в панели, а не в пульсе через минуту после
 * send: apply_failed виден только тому, кто знает, куда смотреть.
 */
/*
 * Инициаторы по исходу: «когда → что сделать». Зеркало Outcome загрузчика
 * инспектора -- расхождение между ними не стиль, а поколение, которое
 * инспектор отвергнет как apply_failed.
 */
function outcomesOf(raw: unknown, phase: string): JsonOutcome[] {
  return arr(raw, `${phase}.outcomes`).map((item, i) => {
    ...
    return {
      ...
      /*
       * Кого писать. Документ, записанный до поля, читается адресом -- так же
       * его читал и загрузчик: прежде поле здесь терялось, и строка «подсеть»
       * из панели доезжала до инспектора адресом.
       */
...
function checkOutcome(outcome: JsonOutcome, i: number, phase: string): void {
  ...
  checkAsk(where, outcome, {
    fail,
    /* Исход бывает отказом: там просьба соседу уже не доедет. */
    onDeny: outcome.on === "deny",
    /* Кадр: записи ответа у него нет. */
...
export function validateDoc(input: unknown): JsonProfileDoc {
  ...
  // Правила приёма чужих просьб: зеркало validatePrior загрузчика инспектора.
  // Проверяются и у выключенного профиля -- опечатка обязана быть видна тогда,
  // когда её сделали.
  doc.trigger.prior.forEach(checkPrior);

  // Инициаторы -- вторая сторона того же канала, и проверяются так же рано.
  ...
  /*
   * Кадры описываются только отдельной схемой: у OpenAPI нет понятия
   * сообщения в сокете. Зеркало validateFrame загрузчика инспектора.
   */
 ...
 * (inspectors/json/internal/config/profile.go): оба глагола умеют ослаблять --
 * skip всегда, у threshold знак (скидку) выбирает отправитель на проводе, --
 * поэтому имя отправителя обязательно, а широковещательного правила у этого
 * инспектора не бывает вовсе.
 */
 ...
 *
 * sources переводит uuid объекта в его имя: на ноде схема лежит файлом
 * schema-<имя> рядом с profile.yaml, и второе имя у одного документа
 * разъехалось бы при первом переименовании.
 */
export function renderProfileYaml(
   ...
   * профиль без единого правила просто никого не слушает. */
  ...
  /*
   * Кадры печатаются только включёнными: у обычного API сокетов нет, и
   * секция с enabled: false в каждом профиле читалась бы как настройка,
   * которой там нет.
   */
  if (doc.frame.enabled) {
    out.push("frame:");
...
/**
 * Таблица политик печатается строкой на исход и в том же порядке, что стоит в
 * панели: правило читают там, где его нашли глазами, а не там, где его удобнее
 * было сериализовать. Счёт печатается только у `score` -- у отказа и пропуска
 * он ничего не значит, и держать его в файле значит приглашать спор о числе,
 * которое никто не применяет.
 */
/*
 * Инициаторы по исходу. Пустые поля не печатаются вовсе: строка профиля
 * читается глазами, и «delta: 0» рядом с записью в набор объясняла бы то,
 * чего в ней нет.
 */
...
/*
 * size печатает байты человеческой записью -- той самой, которую разбирает
 * ParseSize инспектора. Не круглое число печатается как есть: "1048577" честнее
 * округления, из-за которого предел молча уехал бы.
 */

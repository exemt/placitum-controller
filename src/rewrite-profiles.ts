/*
 * Профили инспектора rewrite в Postgres.
 *
 ...
 * каждое новое поле профиля. Проверка -- rewrite-profile-doc.ts, до записи.
 */
...
export class RewriteProfileRepo {
  ...
  /*
   * Имена инспекторов контура: GET профиля помечает отправителей правил
   * приёма, которых в реестре нет. Замечание, а не отказ.
   */
  ...
  /*
   * Записи каталога отказов: профиль называет одну из них, и опечатка
   * обязана ловиться на PUT, а не apply_failed в пульсе через минуту.
   */

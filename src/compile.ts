...
/** Тела поколения с его хешем: то, что keep-alive кладёт обратно в Redis. */
...
/**
 * План -- чистая половина компилятора: источник, пакет и хеш, без единой
 * записи ни в Redis, ни в KV, ни на диск. Из него растут две вещи: `send`
 * (план плюс публикация) и сверка «сохранённое против изданного».
 *
 * Второй реализации хеша здесь быть не должно ни при каких условиях. Она уже
 * была -- на превью конфига в панели -- и разъехалась с первой; заметка стоит
 * в ux/src/config/PreviewDock.tsx. Поэтому сверка зовёт `plan`, а `send` --
 * `plan` и следом публикацию, и разойтись им негде.
 */
export interface RulesPlan {
  sha256: string;
  profiles: string[];
  source: Awaited<ReturnType<RuleSetRepo["exportCompile"]>>;
  /** Настройки процесса из каталога: часть источника канала, входят в хеш. */
...
export class RulesCompiler {
  ...
  /**
   * Тела поколения из плана -- для keep-alive: по хешу он узнаёт, то ли это
   * поколение, что опубликовано, и кладёт блобы обратно, если Redis их
   * потерял. Ни одной записи: план чистый.
   */
  ...
  async compile(httpSpaceId: string): Promise<RuleCompilePublished> {
    ...
    /*
      Блобы кладутся до сверки хеша, а не после: SET NX по уже лежащему ключу
      ничего не стоит, а внутренний Redis живёт без диска -- после его
      перезапуска указатель в KV остаётся, тел за ним нет, и send с тем же
      хешем обязан их вернуть, иначе новая нода не соберётся никогда.

      Срок -- BLOB_TTL_SEC, как у ip: дальше его продлевает keep-alive, пока
      указатель опубликован. Повторный send того же хеша срок тоже обновляет.
    */
    ...
    if (current !== null && current.sha256 === packed.sha256) {
      // Указатель тот же, но рассылка состоялась: см. markPublished.
...
export interface IpPlan {
  ...
  /** Сколько стоил план: `packIp` идёт по всем наборам адресов. */
...
export class IpCompiler {
  ...
  /** Тела поколения из плана -- для keep-alive, см. RulesCompiler.blobItems. */
  async blobItems(httpSpaceId: string): Promise<BlobItems> {
    const { packed } = await this.plan(httpSpaceId);
...
// ---------- Nginx config compiler ----------

/**
 * Содержимое пространства уезжает на ноду целиком, а не по ссылкам: каталог
 * заменяется атомарно, и объект, убранный из пространства, исчезает с ноды.
 *
 * Имя файла считает экспорт -- имя набора плюс расширение по типу. Шаблон
 * ищет страницу отказа как `pages:/$waf_deny_name.html`, поэтому имя набора и
 * есть имя записи каталога, ради которой его завели.
 */
...
export interface NginxPlan {
  ...
  /**
   * Какой профиль какого инспектора называет шаблон. Не для печати: по этой
   * карте межканальная проверка ловит шаблон, сославшийся на профиль, который
   * канал профилей ещё не издал, -- инспектор отвечает на такой тег вердиктом
   * `error`, и маршрут встал бы в `waf_exception … inspector` при двух зелёных
   * каналах по отдельности.
   */
...
export class NginxCompiler {
  ...
  /**
   * Чистая половина: текст, пакет и хеш. Чтение store из базы здесь есть --
   * шифротекст входит в хеш дерева, -- но записи нет ни одной.
   */
  async plan(source: NginxExport): Promise<NginxPlan> {
    ...
    try {
      ...
      // Отказ компилятора -- тоже состояние конфигурации: `send` отвечает по
      // нему 422, сверка -- «не собирается» с той же причиной. Исключение
      // отсюда было 500 у одного и пустым списком причин у другой.
  ...
  /**
   * Тела поколения из плана -- для keep-alive. null -- план не собрался
   * (ошибки валидации): восстанавливать нечего, и это не сбой keep-alive.
   */
  ...
  async compile(source: NginxExport): Promise<NginxCompilePublished> {
    ...
    // Блобы -- до сверки хеша и со сроком, по той же причине, что у правил
    // выше: send с прежним хешем обязан вернуть тела после перезапуска
    // внутреннего Redis, а срок дальше продлевает keep-alive.
    const put = await this.redis.setNxExpireMany(

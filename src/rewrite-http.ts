/*
 * API профилей инспектора rewrite. Устроен как vlai-http: роутер ходит в
 * репозиторий напрямую; в сборке nginx.conf профили не участвуют -- туда
 * доезжает только сама декларация процесса (nginx-http.ts), а выражения и
 * операции едут инспектору каналом policy/rewrite.
 *
 * Сверх формы документа проверяется одно: запись каталога отказов, которой
 * инспектор отвечает на несостоявшуюся подмену. Опечатка в ней иначе всплыла
 * бы ERROR-строкой модуля на живом отказе -- худший момент из возможных.
 */
...
export function rewriteRouter(
  ...
  /*
   * Названная запись каталога обязана существовать: модуль откатится на
   * waf_deny_response_default с ERROR в лог, и случится это на живом отказе.
   * Проверка здесь, где прочитаны обе стороны.
   */
  ...
  router.get("/profiles/:uuid", async (req: Request, res: Response, next) => {
    try {
      ...
      /*
       * Отправители правил приёма, которых нет в реестре контура: их просьб
       * не будет никогда. Замечание, а не отказ -- реестр правят отдельно.
       */

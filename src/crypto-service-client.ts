/*
 * Клиент к crypto-сервису (crypto/, отдельный процесс с приватным ключом
 * контура). Контроллер сам ciphertext не открывает -- он передаёт scope и
 * UUID store-объектов, а crypto-сервис забирает ciphertext у контроллера
 * тем же вызовом, что и агент, расшифровывает и возвращает метаданные.
 * Полная спецификация -- docs/crypto-service.md.
 */
...
export interface CertificateMetadataInput {
  ...
  certStoreId: string;
  /**
   * Отсутствует у client_ca: у доверенного корня mTLS приватной половины
   * нет, сверять нечего. Тогда crypto-сервис только разбирает сертификат.
   */
  keyStoreId?: string;
  chainStoreId?: string;
}
...
export interface CertificateMetadata {
  ...
  issuer: string;
  serial: string;
  /** BasicConstraints CA:TRUE -- реальный признак, а не выбор в форме. */
  isCa: boolean;
}
...
export class CryptoServiceClient {
  ...
  /**
   * Один транспорт на обе ручки: сеть, таймаут и разбор `error` у них
   * одинаковые, различается только путь и тело.
   */

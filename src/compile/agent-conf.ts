/**
 * Указатель настроек агента в JetStream KV.
 *
 * В отличие от поколения nginx у него нет ни блобов в Redis, ни ссылок
 * `store:<uuid>`: документ мелкий и не секретный -- адрес хранилища, регион,
 * имена бакетов и темп выгрузки. Он целиком лежит в значении ключа, и агент
 * применяет его, ничего больше не запрашивая.
 *
 * Реквизиты сюда не попадают ни при каких условиях: ключи S3 остаются
 * секретом ноды. Документ называет хранилище, но не открывает его.
 */
...
/**
 * Тело документа на проводе. Ключи печатаются в фиксированном порядке: хеш
 * считается по этому тексту, и перестановка ключей не должна выглядеть новой
 * настройкой -- иначе каждая пересборка объекта была бы новым поколением.
 */

/**
 * Чтение пространства из Postgres в NginxExport. Компилятор этот документ
 * не пишет -- только печатает.
 */
...
/**
 * Адреса развёртывания, а не пространства: их знает окружение контроллера --
 * тем же значением подняты агент и инспекторы. В базе их нет намеренно, см.
 * [InfraUrls].
 */
...
/**
 * Один разбор строки сертификата на оба запроса: и на список пространства,
 * и на join привязок. Раскладка колонок у них одинаковая, различаются только
 * псевдонимы `name`/`kind`, которые вызывающий приводит перед вызовом.
 */
...
/*
 * Расширение по типу содержимого: имя файла на ноде -- имя набора плюс оно.
 * Тип `binary` и `other` расширения не получают -- имя набора и есть имя файла.
 */
...
export async function exportNginx(pool: Pool, httpSpaceId: string): Promise<NginxExport> {
  ...
  /*
   * Каталог для компилятора -- имя, subject и фазы. Опции директив берутся из
   * графа `*.waf.inspectors`, а не отсюда: у одного имени они разные на разных
   * уровнях, поэтому копия на строке процесса всё равно ничего не решала.
   */

/**
 * Составные наборы адреса: списки, страны и ASN, минус исключения.
 *
 * Набор ничего не решает -- он отвечает на вопрос «кто это». Что с ним делать,
 * говорит правило профиля, см. docs/profiles.md репозитория ip.
 */
...
/**
 * Пустой набор не совпадает ни с чем, а inverse от него совпадает со всем.
 * И то и другое -- почти наверняка недописанная конфигурация, и поймать её
 * надо на сохранении, а не по трафику.
 */
...
export class IpSetRepo {
  ...
  /**
   * Набор, на который ссылается правило, не удаляют: иначе профиль потерял бы
   * строку молча. Ссылку снимают в профиле, потом удаляют набор.
   */

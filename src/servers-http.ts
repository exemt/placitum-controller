 ...
 *
 * `listens` -- сами адреса, а не только их число: страница показывает, что
 * сервер слушает, и без них ей пришлось бы дёргать `/servers/:uuid/listens`
 * на каждую строку таблицы. `listen_count` остаётся рядом: он считается по
 * привязкам и когда список не передан (создание, удаление), и по нему живёт
 * пагинация.
 */
...
export function serversRouter(
  ...
  /**
   * Порядок путей сервера. Тело -- все его пути в нужной последовательности;
   * неполный или чужой список отклоняется: панель правит порядок по своему
   * снимку, и если пути с тех пор добавили или удалили, снимок устарел.
   */
...
/** Тело `PUT /servers/:uuid/locations/order`: uuid путей без повторов. */

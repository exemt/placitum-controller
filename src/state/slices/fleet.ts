...
/**
 * Пометки контроллера поверх кадра — производитель о них не знает: перекос
 * часов, сбросы uptime (рестарты) и рост reconnects шины за последний час.
 * Считаются от предыдущей записи того же участника; сама запись — setOne,
 * то есть всегда равна последнему кадру плюс эти пометки, и секция, которую
 * производитель перестал слать, не залипает от старых кадров.
 */
function deriveMarks(
  ...
  // 30 с допуска: uptime приходит округлённым и может дрогнуть без рестарта.
...
/** Приросты evicted между кадрами Redis: eviction на сторе — потеря данных. */
...
/**
 * Воркеры — `workers`, агенты — соседняя `agents`. Merge для API — по node_id.
 * Тик ставит degraded, потом удаляет. seq — чтобы сокет слал только дельты.
 */
...
/** Разобрать кадр с шины и положить в секцию воркеров. Чужое — отбросить. */
...
/** Разобрать кадр агента и положить в соседнюю секцию. */
...
/** Разобрать кадр инспектора. Чужое — отбросить. */
...
/** Разобрать кадр Redis. Чужое — отбросить. */
...
/** Разобрать кадр S3. Чужое — отбросить. */
...
/** Разобрать кадр сервиса (logger, geo). Чужое — отбросить. */

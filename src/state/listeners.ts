...
/**
 * Наблюдатели живой модели. Срабатывают после редьюсера: в сторе уже новая
 * строка. Сюда ставят публикацию на шину.
 *
 * Набор адресов -- локальный слой, без reload. Правка каталога/правил --
 * черновик шаблона, на агентов nginx не едет, пока не будет `send`.
 */
export function attachListeners(start: AppStartListening): void {
  ...
  /*
   * Удаление набора: keeper снимает его по define (определения больше нет).
   * Черновику пересчитаться надо тоже: слот waf_local_dataset уходит из шаблона.
   */
  start({
  ...
  /*
   * Записи активного набора уже у keeper -- thunk писал через него. Записи
   * статического набора едут конфигурацией: черновику пересчитаться.
   */
  start({

/**
 * `/ip-sets`: составные наборы адреса.
 *
 * Набор -- сущность каталога, а не конфигурации: правит его оператор, а на
 * контур он уезжает вместе с профилями, поколением ip-pack.
 */
...
export function ipSetsRouter(repo: IpSetRepo): Router {
  ...
  router.delete("/:uuid", async (req: Request, res: Response, next) => {
    try {
      ...
      /*
       * Набор, на который ссылается правило, не удаляют: профиль потерял бы
       * строку молча. Сначала снимают ссылку в профиле.
       */
      if (done === "in_use") {

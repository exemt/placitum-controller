/*
 * Профили счётчика и его общая секция в Postgres.
 *
 * Профиль лежит документом jsonb, как у калитки, капчи и контракта: форма
 * описана в Go и меняется вместе с инспектором. Общая секция -- объявления
 * счётчиков и источники ключей субъектов -- одна строка на пространство
 * (counter_shared): она свойство инспектора, а не профиля, и профили ссылаются
 * на счётчики по имени.
 */
...
export class CounterProfileRepo {
  ...
  /*
   * Общая секция. Строки может не быть -- тогда отдаётся пустая заготовка:
   * счётчиков нет, и send честно откажет missing_shared, а панель покажет
   * пустую таблицу, а не ошибку.
   */
   ...
   * которого в реестре нет, не сработает никогда. Замечание, а не отказ.
   */
  ...
  /** Имена записей каталога отказов пространства: их называет deny_response. */

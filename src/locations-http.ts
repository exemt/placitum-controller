...
export function locationsRouter(
  ...
  router.put("/:uuid", async (req: Request, res: Response, next) => {
    try {
      ...
      /*
       * У корня заперт адрес, а не запись: обработчик, апстрим и оверлеи
       * правятся как у любого пути. Сменить match или path значило бы
       * оставить сервер без корня, а запросы -- без uuid маршрута.
       */
      if (
        current.builtin &&

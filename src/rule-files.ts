...
export class RuleFileRepo {
  ...
  /*
   * Кто держит файл. Две ссылки разной природы: состав профилей
   * (rule_set_files, с FK) и текст соседних файлов -- `@pmFromFile ssrf.data`
   * пак разрешает по имени в каталоге, и словарь, который никто не включает в
   * профиль напрямую, всё равно обязан существовать -- без него компиляция
   * падает целиком. Фильтр -- тем же [referencedDataFiles], что и pack:
   * проверка обязана видеть ровно те ссылки, что увидит компилятор.
   */
  async setUses(
    id: string,
    name: string,

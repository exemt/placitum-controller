/**
 * `/api/log-levels` — уровни журнала сервисов контура (log-levels.ts).
 *
 * Не черновик и не рассылка, в отличие от настроек агента: уровень — ручка
 * расследования, а не конфигурация контура. PUT пишет документ в KV сразу,
 * сервисы применяют его первым же событием watch, без рестарта. Ревизия растёт
 * на каждую правку: по ней строка «log level applied» в журнале сервиса
 * сходится с правкой в панели.
 *
 * Путь не под пространством: процессы общие на весь контур.
 */

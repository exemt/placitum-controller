...
export function createApp(cfg: Config, services: AppServices): Express {
  ...
  // base64(blob) + metadata + тексты правил; потолок blob задаёт storeMaxBytes.
  ...
  /*
    Настройки процесса из каталога -- один источник на все ручки send и на
    планировщик (main.ts): план и рассылка обязаны читать одно и то же.
  */
  ...
  */
  ...
  /*
    ...
    и ручка scoped: каталог у каждого пространства свой.
  */
  scoped.use(
    "/geo",
  ...
  /*
    Сходимость -- не страница, а состояние контура: что сохранено, что издано и
    что применили участники. Один документ на все каналы, потому что шапка
    панели показывает сумму, а не обходит шесть страниц.
  */
  scoped.use(
    "/convergence",
  ...
  /*
    Настройки агента -- документ рядом с nginx, а не внутри него: применяет их
    сам агент, без `nginx -t` и без reload. Поэтому свой роутер и свой `send`.
  */
  scoped.use(
    "/agent",
  ...
  /*
    Настройки haproxy -- третий документ той же формы: контроллер собирает из
    него haproxy.cfg целиком, применяет агент балансировщика.
  */
  scoped.use(
    "/haproxy",
  ...
  app.use((err: unknown, _req: Request, res: Response, _next: NextFunction) => {
    // Ошибка, которая знает свой код, -- это ответ клиенту (занятое имя,
    // ненайденная строка), а не сбой процесса, и логировать её как сбой значит
    // топить настоящие аварии в шуме.
    ...
    // `String(err)` на объекте даёт "[object Object]" -- ошибка, которую
    // нельзя прочитать, стоит дороже двух строк здесь.

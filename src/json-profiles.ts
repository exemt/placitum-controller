/*
 * Профили контракта API в Postgres.
 *
 ...
 * поле профиля. Проверка -- json-profile-doc.ts, до записи.
 *
 * Сама спецификация здесь не лежит: она -- объект содержимого раздела данных,
 * и профиль ссылается на него uuid'ом. Так документ остаётся один на всех, кто
 * им пользуется, и правится там же, где остальные файлы контура.
 */
...
/** Документ содержимого: то, из чего собирается файл схемы в манифесте. */
...
export class JsonProfileRepo {
   ...
   * чем разбором логов. Это замечание, а не отказ.
   */
  ...
  /*
   * Тело объекта содержимого пространства -- спецификация. Тип не проверяется:
   * OpenAPI пишут и в YAML (тип text), и в JSON, и требовать одного из них
   * значило бы запретить половину живых документов. Разбирается документ при
   * сборке манифеста -- там, где ошибка ещё видна оператору.
   */
  ...
  /** Имена записей каталога отказов пространства: их называют политики фаз. */

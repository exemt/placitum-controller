...
/**
 * Три каталога http {}, без которых конфигурация не работает, а страницы у них
 * не было: ответы отказа, обменник объектов и форматы логов.
 *
 * В Redux контроллера они не живут намеренно. Тот стор существует ради
 * listener'ов, публикующих изменения в JetStream (наборы адресов, правила);
 * эти три никуда не публикуются -- их читает только компилятор при `send`.
 * Заводить под них слайс, thunk и listener значило бы копировать машинерию,
 * которой нечего делать.
 */

export const DENY_TYPES = ["http", "grpc", "websocket"] as const;

/**
 * Словарь `params=`: что записи разрешено отдать клиенту. Слова -- хвосты
 * имён переменных `$waf_deny_*` (docs/deny-pages.md); модуль по ним открывает
 * страницу. Порядок здесь -- порядок чипов в панели и слов в печати.
 */
export const DENY_PARAMS = ["ray", "addr", "scope", "subject", "retry"] as const;
/**
 * Драйвер обменника один. `none` и `inline` в модуле остались ради уже
 * написанных `nginx.conf`, горячий путь их не вызывает, и маршрут со снимком
 * или архивом при них не проходит `nginx -t`; выбирать между ними в панели
 * значило бы предлагать сломать контур.
 */
export const STORE_DRIVERS = ["redis"] as const;
/** Только `log_format` nginx: `waf_log_format` снят вместе с `waf_audit`. */
export const LOG_FORMAT_KINDS = ["nginx"] as const;

/** Имя директивы nginx: то, чем на запись ссылаются из маршрута. */
...
export class DenyResponseRepo {
  ...
  /*
   * Кто держит записи каталога по базе: правила ip-профилей (колонка
   * `response`) и документы профилей четырёх подсистем, где имя лежит
   * строкой jsonb. Незаполненное поле -- тоже ссылка: разбор документа
   * подставляет умолчание подсистемы, и в profile.yaml имя печатается
   * всегда, поэтому coalesce повторяет умолчания и порядок чтения
   * (`?? `) из *-profile-doc.ts. Ссылки waf-документов трёх уровней
   * считаются не здесь, а обходом стора (`denyResponseUses`).
   *
   * Один запрос на пространство, ключ -- имя записи: список каталога
   * дополняется занятостью для каждой строки разом.
   */
  async referenceUses(space: string): Promise<Map<string, { at: string; kind: string }[]>> {
    const { rows } = await this.pool.query<{ kind: string; at: string; resp: string }>(
      `select distinct kind, at, resp from (
         ...
         union all
         /*
          * Запись под 403 держит только профиль с допуском по группам: без
          * gate.groups отказа «не та группа» не бывает, и ссылки на неё у
          * такого профиля нет -- иначе auth_forbidden числилась бы занятой
          * у всех калиток разом.
          */
         select 'auth', a.name,

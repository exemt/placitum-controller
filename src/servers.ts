...
export class ServerRepo {
  ...
  /**
   * Сервер и его корень -- одной транзакцией. Корень (`prefix /`, builtin)
   * обязателен: запрос, не попавший в другие пути, попадает в него, и только
   * так у каждой записи аудита есть uuid маршрута. Обработчик у свежего корня
   * -- return 404: апстрима у нового сервера ещё нет, проксировать некуда.
   */

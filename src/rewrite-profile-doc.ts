/*
 * Документ профиля rewrite: нормализация, проверка и печать в YAML.
 *
 * Правила повторяют загрузчик инспектора
 * (inspectors/rewrite/internal/config/profile.go) -- ту же отбраковку и в том
 * же порядке. Расхождение между ними не стиль, а поколение, которое инспектор
 * отвергнет как apply_failed; поймать ошибку оператора надо в панели, а не в
 * пульсе через минуту после send.
 *
 * Выражения проверяются здесь `new RegExp` -- это JS, а исполняет их Go (RE2).
 * Осознанная щель: PCRE-специфику, которой нет в JS, панель отвергнет зря, а
 * JS-специфику пропустит -- её отбракует загрузчик инспектора со статусом
 * apply_failed канала, до живого трафика она не доедет.
 */
...
/* Пределы -- зеркало загрузчика инспектора и модуля. */
...
/*
 * Заголовки, которые группа не трогает, -- зеркало запретов модуля. Общий
 * запретный список канала headers плюс асимметрия фазы ответа: подставить
 * Set-Cookie нельзя (канал cookie), снять -- можно; Content-Type наоборот.
 */
...
export function validateDoc(input: unknown): RewriteProfileDoc {
  ...
  // быть видна тогда, когда её сделали.
...
function checkGroup(group: RewriteGroup, at: string): void {
  ...
  if (group.on === "frame") {
    // Заголовков у кадра нет; статус и тип -- селекторы ответа.
 ...
 * ослаблять -- skip и off снимают маскировку, -- поэтому имя отправителя
 * обязательно, а широковещательного правила не бывает вовсе.
 */
...
export function renderProfileYaml(name: string, doc: RewriteProfileDoc): string {
  ...
  if (doc.groups.length > 0) {
    ...
    for (const group of doc.groups) {
      ...
      /* on печатается только у кадровых групп: старый загрузчик ключа не
       * знает, а умолчание response у него и так. */
      if (group.on === "frame") {
        out.push("    on: frame");
  ...
  /* Секция печатается только с правилами: пустой trigger -- обычное

...
/**
 * Из документа в панель не уходит то, что задаёт не она: `waf_node_id` --
 * идентичность ноды, адрес шины -- развёртывание. Сохранённый адрес у старых
 * пространств в базе остаётся, но панель его не видит и не возвращает при
 * сохранении.
 */
...
/**
 * Адреса развёртывания, с которыми поднят контроллер: шина и оба Redis --
 * обменник тел (`CONTROLLER_REDIS_URL`) и внутренний контура
 * (`CONTROLLER_REDIS_INTERNAL_URL`). Панель их показывает, но не правит.
 */
...
/** Пароль из адреса в панель не уходит: `redis://u:***@host`. */
...
export function jsonSpaceHttp(row: HttpSpace, infra: SpaceInfra = {}) {
  return {
    ...
    waf: row.waf,
    /** Адреса развёртывания: панель их показывает, но не правит. */
    infra: {
